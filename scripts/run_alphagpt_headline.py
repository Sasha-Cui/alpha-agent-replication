#!/usr/bin/env python3
"""Evaluate Alpha-GPT's first valid formula on the U.S./JKP contract."""
...
CANDIDATE_ID = "alphagpt_volume_price_correlation_20"
SOURCE_FORMULA = "zscore_scale(ts_corr(close, volume, 20))"
...
def rolling_price_volume_correlation(history: pd.DataFrame) -> pd.DataFrame:
    """Apply the declared conventional operator semantics without future data."""
    ...
    correlation = pd.Series(np.nan, index=frame.index, dtype=float)
    for _, group in frame.groupby("security_id", sort=False):
        correlation.loc[group.index] = group.close.rolling(20, min_periods=20).corr(group.volume).to_numpy()
    frame["correlation"] = correlation
    return frame[["security_id", "month", "correlation"]]


def load_score_history(source_path: Path, start: str, end: str) -> pd.DataFrame:
    start_month = (pd.Timestamp(start) - pd.offsets.MonthEnd(25)).date()
    end_month = pd.Timestamp(end).date()
    raw = pd.read_parquet(
        source_path, columns=["id", "eom", "prc", "tvol"],
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M068 run already exists; do not silently overwrite it")
    ...
    if contract["status"] != "frozen" or recipe["candidate_id"] != CANDIDATE_ID:
        raise ValueError("frozen Alpha-GPT recipe/contract mismatch")
    if recipe["source_formula"] != SOURCE_FORMULA or recipe["source_formula_id"] != "AGPT-FORM-02":
        raise ValueError("source-selected formula changed")
    ...
    evidence = {
        ...
        recipe["paper_audit"]["formula_conformance_path"]: recipe["paper_audit"]["formula_conformance_sha256"],
    }
    for relative, expected in evidence.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned Alpha-GPT evidence changed: {relative}")
    implementation = [
    ...
    raw_score = rolling_price_volume_correlation(history)
    formed = formed.merge(raw_score, on=["security_id", "month"], how="left", validate="one_to_one")
    mean = formed.correlation.groupby(formed.month, sort=False).transform("mean")
    std = formed.correlation.groupby(formed.month, sort=False).transform(lambda x: x.std(ddof=0)).replace(0.0, np.nan)
    formed["score"] = (formed.correlation - mean) / std
    private_dir = root / "artifacts/us_jkp_headline/v1"
    private_dir.mkdir(parents=True, exist_ok=True)
    os.chmod(private_dir, 0o700)
    holdings_path = private_dir / "M068_formation_holdings.parquet"
    ...
    if not np.isfinite(y).all() or (y <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV Alpha-GPT path")
    ...
    verdict = f'''# M068: Alpha-GPT volume-price formula on monthly U.S./JKP data

Status: **completed partial monthly U.S./JKP evaluation**, not the Alpha-GPT interactive mining system.

The first arity-valid source formula `{SOURCE_FORMULA}` is evaluated in its literal orientation. The invalid first Flow-of-Funds expression is not silently repaired. One source bar becomes one month, and conventional rolling-correlation/population-z-score semantics plus the common U.S. top-1,000 value-weighted deciles are disclosed adaptations. Prior outcomes were seen, so inference is exploratory.
...
This result applies only to one materially cadence-adapted formula example. It does not reproduce human interaction, LLM generation, GP search, the generated alpha set, native data/evaluation, or any paper result.
'''
    ...
    manifest = {
        "status": "evaluated_partial", "milestone_id": "M068", "candidate_id": CANDIDATE_ID,
        ...
        "primary_result": primary, "source_formula_id": "AGPT-FORM-02", "source_direction": "literal_positive_orientation",
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("paper_runs/us_jkp_headline/M068_alphagpt"))
    ...
    with (private_dir / "operation.lock").open("a+") as lock:
        ...
        status = {
            "state": "running", "phase": "run", "milestone_id": "M068", "pid": os.getpid(),
