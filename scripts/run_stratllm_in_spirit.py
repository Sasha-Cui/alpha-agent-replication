#!/usr/bin/env python3
"""Run the frozen M054 Strat-LLM-inspired alignment policy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M054 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("Strat-LLM recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("Strat-LLM result must remain explicitly in-spirit")
    alignment_policy = recipe["alignment_policy"]
    if alignment_policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("Strat-LLM recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["strategy_specification_path"]: recipe["strict_evidence"][
            "strategy_specification_sha256"
        ],
        recipe["strict_evidence"]["procedure_component_path"]: recipe["strict_evidence"]["procedure_component_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned Strat-LLM evidence changed: {relative}")
    ...
    if audit["attributable_native_implementation_recovered"] is not False:
        raise ValueError("pinned Strat-LLM implementation status changed")
    if provenance["arxiv"]["id"] != "2605.06024":
        raise ValueError("pinned Strat-LLM paper identity changed")
    ...
    extended_settings["formation_start"] = alignment_policy["prehistory_formation_start"]
    multi_source_state = {
        source: [(item["column"], int(item["sign"])) for item in specifications]
        for source, specifications in recipe["multi_source_state"].items()
    }
    expert_strategy_taxonomy = {
        strategy: [(item["column"], int(item["sign"])) for item in specifications]
        for strategy, specifications in recipe["expert_strategy_taxonomy"].items()
    }
    features = list(
        dict.fromkeys(
            feature
            for specifications in [*multi_source_state.values(), *expert_strategy_taxonomy.values()]
            for feature, _ in specifications
        )
    )
    if alignment_policy["reward_purge_months"] != 1:
        raise ValueError("Strat-LLM reward purge changed")
    print("loading Strat-LLM prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, alignment_history = stratllm_alignment_scores(
        extended,
        multi_source_state,
        expert_strategy_taxonomy,
        common_start=settings["formation_start"],
        reliability_history_months=alignment_policy["source_reliability_history_months"],
        minimum_source_rankic_months=alignment_policy["minimum_source_rankic_months"],
        reliability_temperature=alignment_policy["reliability_softmax_temperature"],
        free_mode_threshold=alignment_policy["free_mode_threshold"],
        guided_mode_lower_threshold=alignment_policy["guided_mode_lower_threshold"],
        action_hold_threshold=alignment_policy["action_hold_threshold"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(alignment_history) != len(expected_months):
        raise ValueError("Strat-LLM reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M054_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete Strat-LLM path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV Strat-LLM return path")
    ...
    alignment_history.to_csv(output / "alignment_history.csv", index=False)
    ...
    mode_counts = alignment_history["selected_mode"].value_counts().to_dict()
    aggregate_actions = {action: int(alignment_history[f"{action}_count"].sum()) for action in ("buy", "hold", "sell")}
    verdict = f"""# M054: Strat-LLM in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not the native Strat-LLM policy or action path.

The reconstruction preserves the paper's three source streams, four strategy motifs, Free/Guided/Strict alignment modes, regime-dependent scaffolding, and buy/hold/sell semantics. Rolling prior source RankIC supplies the missing model-native reliability signal. Mode counts were {mode_counts}; aggregate diagnostic actions were {aggregate_actions}. No model or mode was promoted after observing the common result.
...
This result answers how one transparent Strat-LLM-inspired alignment policy transfers to the common monthly U.S. universe. It does not reproduce live news or report text, paper-time LLM calls, unreleased prompts, intraday inputs, broker execution, native actions, or the paper's empirical claims.
"""
    (output / "verdict.md").write_text(verdict)
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen Strat-LLM recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "source_count": len(multi_source_state),
        "strategy_count": len(expert_strategy_taxonomy),
        "policy_update_months": len(alignment_history),
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M054_stratllm").resolve()
