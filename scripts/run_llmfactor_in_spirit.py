#!/usr/bin/env python3
"""Run the frozen M010 LLMFactor-inspired sequential predictor on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M010 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("LLMFactor recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("LLMFactor result must remain explicitly in-spirit")
    policy = recipe["chronological_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("LLMFactor recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["prompt_path"]: recipe["strict_evidence"]["prompt_sha256"],
        recipe["strict_evidence"]["method_path"]: recipe["strict_evidence"]["method_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned LLMFactor evidence changed: {relative}")
    ...
    factor_candidates = recipe["factor_stage"]["candidates"]
    peer_features = recipe["relation_stage"]["peer_features"]
    peer_inputs = recipe["relation_stage"]["peer_inputs"]
    features = list(dict.fromkeys([*factor_candidates, *peer_features, *peer_inputs]))
    print("loading LLMFactor prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, policy_history = llmfactor_rolling_scores(
        extended,
        factor_candidates,
        peer_features,
        peer_inputs,
        common_start=settings["formation_start"],
        training_months=recipe["prediction_stage"]["training_window_months"],
        selected_factors=recipe["factor_stage"]["selected_count"],
        ridge_penalty=recipe["prediction_stage"]["ridge_penalty"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("LLMFactor reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M010_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete LLMFactor path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV LLMFactor return path")
    ...
    policy_history.to_csv(output / "prediction_history.csv", index=False)
    ...
    verdict = f"""# M010: LLMFactor in-spirit reconstruction on monthly U.S./JKP data
...
At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks. The most frequently selected factors are {factor_text}.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen LLMFactor recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "factor_candidate_count": len(recipe["factor_stage"]["candidates"]),
        "selected_factor_count": recipe["factor_stage"]["selected_count"],
        "policy_update_months": len(policy_history),
        "average_peer_relation_coverage": float(policy_history["peer_relation_coverage"].mean()),
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M010_llmfactor").resolve()
