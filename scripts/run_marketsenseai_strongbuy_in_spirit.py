#!/usr/bin/env python3
"""Run the frozen M052 MarketSenseAI strongest-buy reconstruction on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M052 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("MarketSenseAI recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("MarketSenseAI result must remain explicitly in-spirit")
    policy = recipe["synthesis_and_recommendation_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("MarketSenseAI recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["paper_version_summary_path"]: recipe["strict_evidence"]["paper_version_summary_sha256"],
        recipe["strict_evidence"]["mechanism_conformance_path"]: recipe["strict_evidence"]["mechanism_conformance_sha256"],
        recipe["strict_evidence"]["specification_gaps_path"]: recipe["strict_evidence"]["specification_gaps_sha256"],
        recipe["strict_evidence"]["published_2026_result_ledger_path"]: recipe["strict_evidence"]["published_2026_result_ledger_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned MarketSenseAI evidence changed: {relative}")
    audit = json.loads((root / recipe["strict_evidence"]["audit_manifest_path"]).read_text())
    if audit["operational_system_source_found"] is not False or audit["native_signal_or_portfolio_outputs_found"] is not False:
        raise ValueError("pinned MarketSenseAI release boundary changed")
    ...
    features = list(
        dict.fromkeys(
            ...
            if feature not in {"ret", "weight"}
        )
    )
    print("loading MarketSenseAI prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, policy_history = marketsenseai_strongbuy_scores(
        extended,
        specialists,
        common_start=settings["formation_start"],
        reliability_months=policy["specialist_reliability_history_months"],
        minimum_rankic_months=policy["minimum_specialist_rankic_months"],
        softmax_temperature=policy["reliability_softmax_temperature"],
        percentile_boundaries=tuple(policy["five_class_percentile_boundaries"]),
        ordinal_values=tuple(policy["ordinal_values"]),
        continuous_tie_break_weight=policy["continuous_tie_break_weight"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("MarketSenseAI reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M052_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete MarketSenseAI path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV MarketSenseAI return path")
    ...
    policy_history.to_csv(output / "signal_history.csv", index=False)
    ...
    action_counts = {
        action: int(policy_history[f"{action}_count"].sum())
        for action in ("strong_sell", "sell", "hold", "buy", "strong_buy")
    }
    verdict = f"""# M052: MarketSenseAI strongest-buy in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not the native live MarketSenseAI recommendation panel.

The reconstruction preserves News, Fundamentals, Dynamics, and Macro specialists, context-dependent synthesis, five ordinal recommendation classes, monthly formation, and a rare strongest-buy tail. JKP scores replace private specialist texts and 60 past RankIC months replace NNLS attribution for adaptive synthesis. Fixed cross-sectional boundaries produce the paper's 7.5% strongest-buy share without using published stock identities. The common upper decile represents strongest buys while the lower decile supplies the required apples-to-apples short leg.

At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks. Aggregate classes were {action_counts}.
...
This result answers how one transparent MarketSenseAI strongest-buy reconstruction transfers to the common task. It does not reproduce or validate the live theses, embeddings, fixed cohorts, native long-only holdings, or published results.
"""
    (output / "verdict.md").write_text(verdict)
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen MarketSenseAI recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M052_marketsenseai").resolve()
