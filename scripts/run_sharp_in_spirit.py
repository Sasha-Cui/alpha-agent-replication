#!/usr/bin/env python3
"""Run the frozen M055 SHARP-inspired evolved rubric on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M055 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("SHARP recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("SHARP result must remain explicitly in-spirit")
    evolution_policy = recipe["evolution_policy"]
    if evolution_policy["final_common_returns_used_for_rubric_choice"] is not False:
        raise ValueError("SHARP recipe permits final-result selection")
    ...
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned SHARP evidence changed: {relative}")
    ...
    if audit["attributable_sharp_implementation_found"] is not False:
        raise ValueError("pinned SHARP implementation status changed")
    if provenance["arxiv_id"] != "2605.06822":
        raise ValueError("pinned SHARP paper identity changed")
    ...
    analyst_state = recipe["analyst_state"]
    features = list(
        dict.fromkeys(
            [
                item["column"]
                for stream in ("price", "catalyst", "quality", "attention")
                for item in analyst_state[stream]
            ]
            + [analyst_state["fear_proxy_column"]]
        )
    )
    print("loading SHARP prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, block_history, evolution_history = sharp_evolved_rubric_scores(
        extended,
        analyst_state,
        recipe["initial_rubric"],
        recipe["condition_thresholds"],
        ...
        evolution_rounds=evolution_policy["evolution_rounds"],
        worst_training_months=evolution_policy["worst_training_months"],
        minimum_recurring_pattern_months=evolution_policy["minimum_recurring_pattern_months"],
        pattern_activation_share=evolution_policy["pattern_activation_share"],
        maximum_atomic_mutations_per_round=evolution_policy["maximum_atomic_mutations_per_round"],
        maximum_rubric_rules=evolution_policy["maximum_rubric_rules"],
        validation_tolerance_total_return=evolution_policy["validation_tolerance_total_return"],
    ...
    if formed["month"].nunique() != len(expected_months) or len(block_history) != 13:
        raise ValueError("SHARP reconstruction does not cover the fixed formation calendar")
    if len(evolution_history) != 13 * evolution_policy["evolution_rounds"]:
        raise ValueError("SHARP reconstruction has an incomplete evolution trace")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M055_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete SHARP path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV SHARP return path")
    ...
    block_history.to_csv(output / "rubric_block_history.csv", index=False)
    evolution_history.to_csv(output / "rubric_evolution_history.csv", index=False)
    ...
    accepted_mutation_rounds = int(evolution_history["accepted"].sum())
    best_update_rounds = int(evolution_history["best_update"].sum())
    distinct_frozen_rubrics = int(block_history["frozen_rubric"].nunique())
    verdict = f"""# M055: SHARP in-spirit reconstruction on monthly U.S./JKP data
...
The reconstruction preserves predicted return times confidence, six structured condition-action rules, independent walk-forward initialization, five rounds of worst-tail attribution, at most three atomic edits per round, the 50 bp validation gate, and a frozen best rubric for each test block. Across 13 blocks, {accepted_mutation_rounds} of 65 mutation rounds passed the gate, {best_update_rounds} set a strictly better validation rubric, and {distinct_frozen_rubrics} distinct rubrics were frozen. No common-result retuning occurred.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen SHARP recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
        "primary_monthly_returns.csv",
        "metrics.csv",
        "attribution_residuals.csv",
        "rubric_block_history.csv",
        "rubric_evolution_history.csv",
        "verdict.md",
    ]
    manifest = {
        ...
        "initial_rule_count": len(recipe["initial_rubric"]),
        "walk_forward_blocks": len(block_history),
        "evolution_rounds": len(evolution_history),
        "accepted_mutation_rounds": accepted_mutation_rounds,
        "best_update_rounds": best_update_rounds,
        "distinct_frozen_rubrics": distinct_frozen_rubrics,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M055_sharp").resolve()
