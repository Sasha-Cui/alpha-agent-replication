#!/usr/bin/env python3
"""Run the frozen M012 AAPM-inspired hybrid pricing model on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M012 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("AAPM recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("AAPM result must remain explicitly in-spirit")
    policy = recipe["chronological_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("AAPM recipe permits final-result selection")
    ...
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned AAPM evidence changed: {relative}")
    ...
    extended_settings["formation_start"] = recipe["hybrid_model"]["prehistory_formation_start"]
    report_features = recipe["stock_report_proxy"]
    manual_factors = recipe["manual_factors"]
    asset_features = recipe["asset_embedding_proxy"]
    features = list(dict.fromkeys([*report_features, *manual_factors, *asset_features]))
    print("loading AAPM prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, policy_history, feature_catalog = aapm_rolling_scores(
        extended,
        report_features,
        manual_factors,
        asset_features,
        common_start=settings["formation_start"],
        report_memory_weight=recipe["hybrid_model"]["report_memory_weight"],
        pretraining_months=recipe["hybrid_model"]["pretraining_window_months"],
        ridge_penalty=recipe["hybrid_model"]["ridge_penalty"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("AAPM reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M012_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete AAPM path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV AAPM return path")
    ...
    policy_history.to_csv(output / "pretraining_history.csv", index=False)
    feature_catalog.to_csv(output / "hybrid_feature_catalog.csv", index=False)
    ...
    verdict = f"""# M012: AAPM in-spirit reconstruction on monthly U.S./JKP data
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen AAPM recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "hybrid_feature_count": len(feature_catalog),
        "policy_update_months": len(policy_history),
        "average_stock_report_coverage": float(policy_history["stock_report_coverage"].mean()),
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M012_aapm").resolve()
