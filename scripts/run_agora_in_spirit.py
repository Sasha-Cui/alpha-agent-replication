#!/usr/bin/env python3
"""Run the frozen M061 AGORA-inspired sealed joint search on U.S./JKP."""
...
from alpha_evolve.in_spirit import agora_sealed_joint_search_scores
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M061 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("AGORA recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("AGORA result must remain explicitly in-spirit")
    search_policy = recipe["search_policy"]
    if search_policy["common_or_post_training_returns_used_for_search_or_selection"] is not False:
        raise ValueError("AGORA recipe permits common-period feedback")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["metric_execution_path"]: recipe["strict_evidence"]["metric_execution_sha256"],
        recipe["strict_evidence"]["release_search_path"]: recipe["strict_evidence"]["release_search_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned AGORA evidence changed: {relative}")
    ...
    if audit["attributable_agora_implementation_found"] is not False:
        raise ValueError("pinned AGORA implementation status changed")
    if provenance["arxiv_id"] != "2606.29194":
        raise ValueError("pinned AGORA paper identity changed")
    ...
    extended_settings["formation_start"] = search_policy["prehistory_formation_start"]
    grammar = recipe["alpha_grammar"]
    society = recipe["sealed_society"]
    features = grammar["seed_features"]
    print("loading AGORA training and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, alpha_registry, round_history = agora_sealed_joint_search_scores(
        extended,
        features,
        search_policy["alpha_panel_weights"],
        society["metric_proposal_rounds"],
        common_start=settings["formation_start"],
        training_months=search_policy["training_months"],
        outer_rounds=search_policy["outer_rounds"],
        target_unique_alphas=grammar["target_unique_alphas"],
        final_top_alpha_count=grammar["final_top_alpha_count"],
        exploration_every_rounds=search_policy["epsilon_exploration_every_rounds"],
        auto_promotion_absolute_correlation=society["auto_promotion_absolute_correlation"],
        demotion_absolute_correlation=society["demotion_absolute_correlation"],
        minimum_promotion_observations=society["minimum_promotion_observations"],
        monotonicity_panel_accepts_positive_correlation=society[
            "monotonicity_agent_initiated_panel_accepts_positive_correlation"
        ],
        ppo_relay_decay=search_policy["ppo_relay_decay"],
        ppo_relay_learning_rate=search_policy["ppo_relay_learning_rate"],
        inner_tail_fraction=search_policy["inner_tail_fraction"],
        inner_minimum_side=search_policy["inner_minimum_side"],
        inner_cost_bps_one_way=search_policy["inner_cost_bps_one_way"],
        metric_quantile_groups=search_policy["metric_quantile_groups"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or scores.notna().sum() != len(formed):
        raise ValueError("AGORA reconstruction does not cover the fixed formation calendar")
    if len(alpha_registry) != grammar["target_unique_alphas"] or len(round_history) != search_policy["outer_rounds"]:
        raise ValueError("AGORA reconstruction has an incomplete sealed-search trace")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M061_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete AGORA path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV AGORA return path")
    ...
    alpha_registry.to_csv(output / "alpha_registry.csv", index=False)
    round_history.to_csv(output / "round_history.csv", index=False)
    ...
    selected_alphas = alpha_registry.loc[alpha_registry["selected_top30"], "alpha"].tolist()
    final_accepted_metrics = [name for name in str(round_history.iloc[-1]["accepted_metrics"]).split("|") if name]
    metric_action_counts = round_history["metric_action"].value_counts().to_dict()
    top_component_counts: dict[str, int] = {}
    for value in alpha_registry.loc[alpha_registry["selected_top30"], "components"]:
        for component in value.split("|"):
            top_component_counts[component] = top_component_counts.get(component, 0) + 1
    verdict = f"""# M061: AGORA in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not the native AGORA LLM society, AlphaGen registry, or PPO policy.

The reconstruction preserves a five-role sealed topology, three typed channels, eight persistent skill stores, 100 round-close transitions, a 94-alpha AlphaGen-style registry, evaluator panels, metric-library proposals, and a PPO-style persistent proposal relay. Final accepted evolved metrics were {final_accepted_metrics}; metric actions were {metric_action_counts}. The final 30 alphas were selected only by pre-common training RankIC and combined by equal-weight cross-sectional z-score. No common-period feedback entered any role, promotion, or selection decision.
...
This result answers how one transparent AGORA-inspired sealed joint search transfers to the common monthly U.S. universe. It does not reproduce CSI1000 data, native prompts, 94 released programs, per-round records, LLM adjudication, MaskablePPO trajectories, checkpoints, actions, or paper empirical claims.
"""
    (output / "verdict.md").write_text(verdict)
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen AGORA recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "outer_rounds": len(round_history),
        "alpha_registry_size": len(alpha_registry),
        "selected_top_alpha_count": int(alpha_registry["selected_top30"].sum()),
        "selected_top_alphas": selected_alphas,
        "selected_top_component_counts": top_component_counts,
        "final_accepted_metrics": final_accepted_metrics,
        "metric_action_counts": metric_action_counts,
        "final_metric_store_state": round_history.iloc[-1]["metric_store_state"],
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M061_agora").resolve()
