#!/usr/bin/env python3
"""Run the frozen M047 BlindTrade-inspired graph-intent policy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M047 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("BlindTrade recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("BlindTrade result must remain explicitly in-spirit")
    blind_policy = recipe["blindtrade_policy"]
    if blind_policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("BlindTrade recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["prompt_schema_path"]: recipe["strict_evidence"]["prompt_schema_sha256"],
        recipe["strict_evidence"]["published_result_ledger_path"]: recipe["strict_evidence"]["published_result_ledger_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned BlindTrade evidence changed: {relative}")
    ...
    if provenance["attributable_blindtrade_release_found"] is not False:
        raise ValueError("pinned BlindTrade release status changed")
    if audit["attributable_code_or_data_release_found"] is not False:
        raise ValueError("pinned BlindTrade audit release status changed")
    ...
    extended_settings["formation_start"] = blind_policy["prehistory_formation_start"]
    anonymized_agents = {
        agent: [(item["column"], int(item["sign"])) for item in specifications]
        for agent, specifications in recipe["anonymized_agents"].items()
    }
    features = list(
        dict.fromkeys(
            feature
            for specifications in anonymized_agents.values()
            for feature, _ in specifications
        )
    )
    print("loading BlindTrade prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, policy_history = blindtrade_graph_intent_scores(
        extended,
        anonymized_agents,
        recipe["intent_role_weights"],
        common_start=settings["formation_start"],
        agent_ic_history_months=blind_policy["agent_ic_history_months"],
        minimum_agent_ic_months=blind_policy["minimum_agent_ic_months"],
        agent_reward_purge_months=blind_policy["agent_reward_purge_months"],
        semantic_similarity_threshold=blind_policy["semantic_similarity_threshold"],
        semantic_neighbors=blind_policy["semantic_neighbors"],
        semantic_graph_layers=blind_policy["semantic_graph_layers"],
        graph_self_weight=blind_policy["graph_self_weight"],
        graph_neighbor_weight=blind_policy["graph_neighbor_weight"],
        intent_reward_history_months=blind_policy["intent_reward_history_months"],
        minimum_intent_reward_months=blind_policy["minimum_intent_reward_months"],
        intent_reward_purge_months=blind_policy["intent_reward_purge_months"],
        intent_tie_break_order=tuple(blind_policy["intent_tie_break_order"]),
        reward_cost_bps_one_way=settings["primary_cost_bps_one_way"],
        execution_inertia_eta=blind_policy["execution_inertia_eta"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("BlindTrade reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M047_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete BlindTrade path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV BlindTrade return path")
    ...
    policy_history.to_csv(output / "policy_history.csv", index=False)
    ...
    intent_counts = {
        intent: int(policy_history["selected_intent"].eq(intent).sum())
        for intent in recipe["intent_role_weights"]
    }
    mean_neighbors = float(policy_history["mean_semantic_neighbors"].mean())
    zero_neighbors = int(policy_history["zero_neighbor_count"].sum())
    mean_proposed_score = float(policy_history["mean_absolute_proposed_score"].mean())
    mean_inertial_score = float(policy_history["mean_absolute_inertial_score"].mean())
    stale_resets = int(policy_history["stale_score_resets"].sum())
    mean_agent_reliability = {
        agent: float(policy_history[f"agent_reliability__{agent}"].mean())
        for agent in anonymized_agents
    }
    verdict = f"""# M047: BlindTrade in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not the native Gemini/SemGAT/PPO-DSR BlindTrade system.

The reconstruction preserves identity-blind inputs; Momentum, News-Event, Mean-Reversion, and Risk-Regime roles; causal IC validation; a two-layer similarity graph; defensive, neutral, and aggressive intents; trailing risk-adjusted market feedback; 10 bp turnover cost; and 0.10 execution inertia. Deterministic JKP role scores replace unreleased LLM outputs, their four-score vector replaces reasoning embeddings, cosine-neighbor averaging replaces learned GATv2, and the best trailing 60-month net Sharpe among three fixed intent sleeves replaces PPO-DSR. Intent counts were {intent_counts}; mean semantic neighbors per stock were {mean_neighbors:.2f}; mean absolute score moved from {mean_proposed_score:.3f} before inertia to {mean_inertial_score:.3f} after inertia.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen BlindTrade recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "agent_count": len(anonymized_agents),
        "semantic_graph_layers": blind_policy["semantic_graph_layers"],
        "policy_update_months": len(policy_history),
        "intent_counts": intent_counts,
        "mean_semantic_neighbors": mean_neighbors,
        "zero_semantic_neighbors": zero_neighbors,
        "mean_absolute_proposed_score": mean_proposed_score,
        "mean_absolute_inertial_score": mean_inertial_score,
        "stale_score_resets": stale_resets,
        "mean_agent_reliability": mean_agent_reliability,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M047_blindtrade").resolve()
