#!/usr/bin/env python3
"""Run the frozen M067 RAPTOR-inspired orchestration on monthly U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M067 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("RAPTOR recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("RAPTOR result must remain explicitly in-spirit")
    if recipe["anti_leakage"]["m067_common_result_seen_before_recipe_freeze"] is not False:
        raise ValueError("RAPTOR recipe does not seal the common result")
    ...
    pinned = {
        evidence["audit_manifest_path"]: evidence["audit_manifest_sha256"],
        evidence["source_provenance_path"]: evidence["source_provenance_sha256"],
        evidence["method_specification_path"]: evidence["method_specification_sha256"],
        evidence["native_execution_path"]: evidence["native_execution_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned RAPTOR evidence changed: {relative}")
    ...
    native = json.loads((root / evidence["native_execution_path"]).read_text())
    if audit["author_result_snapshots"] != 166 or audit["end_to_end_result_cells_reproduced"] != 0:
        raise ValueError("pinned RAPTOR native-result boundary changed")
    if provenance["author_repository_head"] != recipe["paper_source"]["author_repository_commit"]:
        raise ValueError("pinned RAPTOR repository revision changed")
    if provenance["ceur_pdf_sha256"] != recipe["paper_source"]["official_pdf_sha256"]:
        raise ValueError("pinned RAPTOR paper changed")
    if native["end_to_end_reason"] != "unreleased_inputs_and_request_logs_plus_conflicting_paper_and_runner_protocols":
        raise ValueError("pinned RAPTOR native execution boundary changed")
    ...
    allocator = recipe["black_litterman_policy"]
    extended_settings = dict(settings)
    extended_settings["formation_start"] = orchestration["prehistory_formation_start"]
    cards = {
        name: [(item["column"], int(item["sign"])) for item in card]
        for name, card in recipe["analyst_signal_cards"].items()
    }
    features = sorted({column for card in cards.values() for column, _ in card})
    print("loading RAPTOR covariance prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, orchestration_history = raptor_orchestrated_bl_scores(
        extended,
        cards,
        orchestration["analyst_weights"],
        common_start=settings["formation_start"],
        covariance_history_months=orchestration["covariance_history_months"],
        minimum_covariance_months=orchestration["minimum_covariance_months"],
        analyst_action_threshold=orchestration["analyst_action_threshold"],
        final_action_threshold=orchestration["final_action_threshold"],
        risk_aversion=allocator["risk_aversion"],
        tau=allocator["tau"],
        omega_scale=allocator["omega_scale"],
        view_magnitude_annualized=allocator["categorical_annualized_views"]["BUY"],
        maximum_long_only_weight=allocator["maximum_long_only_weight"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or scores.notna().sum() != len(formed):
        raise ValueError("RAPTOR reconstruction does not cover the fixed formation calendar")
    if len(orchestration_history) != len(expected_months):
        raise ValueError("RAPTOR reconstruction lacks synchronized monthly diagnostics")
    if not orchestration_history["finite_score_count"].eq(
        orchestration_history["security_count"]
    ).all():
        raise ValueError("RAPTOR posterior score coverage is incomplete")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M067_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete RAPTOR path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV RAPTOR return path")
        ...
        row = {
            ...
            **{
            ...
            "maximum_missing_forward_gross_weight": float(
                candidate.missing_forward_return_gross_weight.max()
            ),
        }
        metrics.append(row)
    ...
    orchestration_history.to_csv(output / "orchestration_history.csv", index=False)
    ...
    final_counts = {
        action: int(orchestration_history[f"final_{action}"].sum())
        for action in ("BUY", "HOLD", "SELL")
    }
    verdict = f"""# M067: RAPTOR in-spirit reconstruction on monthly U.S./JKP data
...
The reconstruction preserves six analyst threads, bull/bear synthesis, research management, three risk stances, a final risk judgment, synchronized categorical views, and the released Black-Litterman constants. Across 305 monthly cross sections the final judge produced {final_counts}; every view used only contemporaneous JKP characteristics and lagged return variance.

At 10 bp one-way costs, the 305-month common path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen RAPTOR recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
        "primary_monthly_returns.csv",
        "metrics.csv",
        "attribution_residuals.csv",
        "orchestration_history.csv",
        "verdict.md",
    ]
    manifest = {
        ...
        "formation_months": len(orchestration_history),
        "agent_roles_per_security": 14,
        "total_append_only_messages": int(orchestration_history["append_only_message_count"].sum()),
        "final_action_counts": final_counts,
        "average_prior_annualized": float(orchestration_history["prior_mean_annualized"].mean()),
        "average_view_annualized": float(orchestration_history["view_mean_annualized"].mean()),
        "average_posterior_annualized": float(
            orchestration_history["posterior_mean_annualized"].mean()
        ),
        "maximum_diagnostic_long_only_weight": float(
            orchestration_history["long_only_maximum_weight"].max()
        ),
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M067_raptor").resolve()
