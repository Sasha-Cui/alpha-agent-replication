#!/usr/bin/env python3
"""Run the frozen M008 Alpha-GPT 2.0-inspired research pipeline on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M008 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("Alpha-GPT 2.0 recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("Alpha-GPT 2.0 result must remain explicitly in-spirit")
    policy = recipe["chronological_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("Alpha-GPT 2.0 recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["lineage_path"]: recipe["strict_evidence"]["lineage_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned Alpha-GPT 2.0 evidence changed: {relative}")
    ...
    seeds = recipe["mining_stage"]["seed_features"]
    risk_features = recipe["analysis_stage"]["risk_features"]
    features = list(dict.fromkeys([*seeds, *risk_features]))
    print("loading Alpha-GPT 2.0 prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, policy_history, candidate_catalog = alpha_gpt2_rolling_scores(
        extended,
        seeds,
        risk_features,
        common_start=settings["formation_start"],
        training_months=policy["training_window_months"],
        minimum_rankic_months=policy["minimum_finite_rankic_months"],
        selected_factors=recipe["mining_stage"]["selected_factors"],
        ridge_penalty=recipe["modeling_stage"]["ridge_penalty"],
        high_risk_quantile=recipe["analysis_stage"]["high_risk_quantile"],
        high_risk_multiplier=recipe["analysis_stage"]["high_risk_score_multiplier"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("Alpha-GPT 2.0 reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M008_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete Alpha-GPT 2.0 path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV Alpha-GPT 2.0 return path")
    ...
    policy_history.to_csv(output / "pipeline_history.csv", index=False)
    candidate_catalog.to_csv(output / "candidate_catalog.csv", index=False)
    ...
    factor_counts = pd.concat(
        [policy_history[f"factor_{number}"] for number in range(1, 6)],
        ignore_index=True,
    ).value_counts().head(3)
    factor_text = ", ".join(f"`{name}` ({int(count)} selections)" for name, count in factor_counts.items())
    verdict = f"""# M008: Alpha-GPT 2.0 in-spirit reconstruction on monthly U.S./JKP data
...
At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks. The most frequently selected mined expressions are {factor_text}.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen Alpha-GPT 2.0 recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "selected_factor_count": recipe["mining_stage"]["selected_factors"],
        "policy_update_months": len(policy_history),
        "average_high_risk_count": float(policy_history["high_risk_count"].mean()),
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M008_alpha_gpt_2_0").resolve()
