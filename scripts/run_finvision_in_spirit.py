#!/usr/bin/env python3
"""Run the frozen M013 FinVision-inspired multi-agent consensus on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M013 in-spirit run already exists; do not silently overwrite it")
    ...
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned FinVision evidence changed: {relative}")
    ...
    features = list(
        dict.fromkeys(
            [
                *recipe["agents"]["news_summary"],
                *recipe["agents"]["technical_chart"],
            ]
        )
    )
    print("loading FinVision prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, policy_history = finvision_consensus_scores(
        extended,
        common_start=settings["formation_start"],
        reliability_months=recipe["prediction_policy"]["reliability_window_months"],
        minimum_rankic_months=recipe["prediction_policy"]["minimum_rankic_months"],
        softmax_temperature=recipe["prediction_policy"]["softmax_temperature"],
        hold_threshold=recipe["prediction_policy"]["action_hold_threshold"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("FinVision reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M013_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete FinVision path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV FinVision return path")
    ...
    policy_history.to_csv(output / "agent_history.csv", index=False)
    ...
    verdict = f"""# M013: FinVision in-spirit reconstruction on monthly U.S./JKP data
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen FinVision recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "upstream_agent_count": len(recipe["agents"]),
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M013_finvision").resolve()
