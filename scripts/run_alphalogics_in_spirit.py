#!/usr/bin/env python3
"""Run the frozen M048 AlphaLogics-inspired factor evolution on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M048 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("AlphaLogics recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("AlphaLogics result must remain explicitly in-spirit")
    evolution_policy = recipe["logic_evolution_policy"]
    if evolution_policy["final_common_returns_used_for_search_or_model_choice"] is not False:
        raise ValueError("AlphaLogics recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["dsl_operation_ledger_path"]: recipe["strict_evidence"]["dsl_operation_ledger_sha256"],
        recipe["strict_evidence"]["algorithm_conformance_path"]: recipe["strict_evidence"]["algorithm_conformance_sha256"],
        recipe["strict_evidence"]["prompt_template_ledger_path"]: recipe["strict_evidence"]["prompt_template_ledger_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned AlphaLogics evidence changed: {relative}")
    ...
    market_logics = {
        logic: [(item["column"], int(item["sign"])) for item in specifications]
        for logic, specifications in recipe["initial_market_logic_library"].items()
    }
    features = list(
        dict.fromkeys(
            feature
            for specifications in market_logics.values()
            for feature, _ in specifications
        )
    )
    print("loading AlphaLogics prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, search_history, search_summary = alphalogics_evolution_scores(
        extended,
        market_logics,
        calibration_train_start=evolution_policy["calibration_train_start"],
        calibration_train_end=evolution_policy["calibration_train_end"],
        calibration_validation_start=evolution_policy["calibration_validation_start"],
        calibration_validation_end=evolution_policy["calibration_validation_end"],
        common_start=settings["formation_start"],
        round_operation_order=recipe["compiled_factor_dsl"]["round_operation_order"],
        outer_rounds=evolution_policy["outer_rounds"],
        inner_candidate_budget_per_logic_round=evolution_policy["inner_candidate_budget_per_logic_round"],
        inner_early_stopping_non_improvements=evolution_policy["inner_early_stopping_non_improvements"],
        minimum_validation_months=evolution_policy["minimum_validation_months"],
        final_ridge_fraction=evolution_policy["final_ridge_fraction"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or scores.notna().sum() != len(formed):
        raise ValueError("AlphaLogics reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M048_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete AlphaLogics path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV AlphaLogics return path")
    ...
    search_history.to_csv(output / "search_history.csv", index=False)
    ...
    selected_text = "; ".join(
        f"{logic}: {expression}"
        for logic, expression in search_summary["selected_expressions"].items()
    )
    early_stops = int(search_history["stopped_early"].sum())
    candidates_evaluated = int(search_history["candidates_evaluated"].sum())
    verdict = f"""# M048: AlphaLogics in-spirit reconstruction on monthly U.S./JKP data
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen AlphaLogics recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "initial_logic_count": len(market_logics),
        "outer_rounds": evolution_policy["outer_rounds"],
        "logic_round_evaluations": len(search_history),
        "candidates_evaluated": candidates_evaluated,
        "early_stops": early_stops,
        "selected_expressions": search_summary["selected_expressions"],
        "selected_orientations": search_summary["selected_orientations"],
        "selected_validation_icir": search_summary["selected_validation_icir"],
        "final_intercept": search_summary["final_intercept"],
        "final_coefficients": search_summary["final_coefficients"],
        "final_ridge_penalty": search_summary["final_ridge_penalty"],
        "calibration_observations": search_summary["calibration_observations"],
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M048_alphalogics").resolve()
