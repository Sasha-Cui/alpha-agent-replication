#!/usr/bin/env python3
"""Run the frozen M030 MM-ARC-inspired RABO capital routing on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M030 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("MM-ARC recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("MM-ARC result must remain explicitly in-spirit")
    routing = recipe["routing_and_audit_policy"]
    if routing["final_common_returns_used_for_pool_or_router_choice"] is not False:
        raise ValueError("MM-ARC recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["lfs_recovery_path"]: recipe["strict_evidence"]["lfs_recovery_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned MM-ARC evidence changed: {relative}")
    provenance = json.loads((root / recipe["strict_evidence"]["source_provenance_path"]).read_text())
    if provenance["arxiv"]["pdf_sha256"]["v3"] != recipe["paper_source"]["pdf_sha256"]:
        raise ValueError("pinned MM-ARC paper identity changed")
    if provenance["arxiv"]["source_sha256"]["v3"] != recipe["paper_source"]["source_sha256"]:
        raise ValueError("pinned MM-ARC paper source changed")
    ...
    extended_settings["formation_start"] = routing["prehistory_formation_start"]
    experts = {
        expert: [(item["column"], int(item["sign"])) for item in specifications]
        for expert, specifications in recipe["expert_candidate_features"].items()
    }
    features = list(
        dict.fromkeys(
            feature
            for specifications in experts.values()
            for feature, _ in specifications
        )
    )
    print("loading MM-ARC prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, routing_history = mm_arc_routing_scores(
        extended,
        experts,
        view_weights=recipe["candidate_schema"]["view_weights"],
        rabo_rank_weights=routing["rabo_rank_weights"],
        common_start=settings["formation_start"],
        audit_history_months=routing["audit_history_months"],
        minimum_regime_months=routing["minimum_regime_months"],
        audit_blocks=routing["purged_block_proxy_count"],
        admitted_per_pool=recipe["candidate_schema"]["admitted_per_pool"],
        router_performance_temperature=routing["router_performance_temperature"],
        router_robustness_tilt=routing["router_robustness_tilt"],
    )
    ...
    if (
        ...
        or len(routing_history) != len(expected_months)
    ):
        raise ValueError("MM-ARC reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M030_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete MM-ARC path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV MM-ARC return path")
    ...
    routing_history.to_csv(output / "routing_history.csv", index=False)
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen MM-ARC recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "single_market_pool_count": recipe["candidate_schema"]["single_market_pool_count"],
        "policy_update_months": len(routing_history),
        "regime_counts": regime_counts,
        "regime_sparse_fallback_months": sparse_fallback_months,
        "mean_router_weights": mean_router_weights,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M030_mm_arc").resolve()
