#!/usr/bin/env python3
"""Run the frozen M053 AlphaCrafter-inspired workflow on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M053 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("AlphaCrafter recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("AlphaCrafter result must remain explicitly in-spirit")
    workflow_policy = recipe["workflow_policy"]
    if workflow_policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("AlphaCrafter recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["release_execution_audit_path"]: recipe["strict_evidence"]["release_execution_audit_sha256"],
        recipe["strict_evidence"]["registered_model_override_execution_path"]: recipe["strict_evidence"]["registered_model_override_execution_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned AlphaCrafter evidence changed: {relative}")
    ...
    if audit["attributable_repository_recovered"] is not True:
        raise ValueError("pinned AlphaCrafter repository status changed")
    if provenance["arxiv"]["id"] != "2605.05580":
        raise ValueError("pinned AlphaCrafter paper identity changed")
    if provenance["attributable_repository"]["head_sha"] != recipe["paper_source"]["repository_commit_checked_2026_09_05"]:
        raise ValueError("pinned AlphaCrafter repository identity changed")
    ...
    extended_settings["formation_start"] = workflow_policy["prehistory_formation_start"]
    miner_clusters = {
        miner: [(item["column"], int(item["sign"])) for item in specifications]
        for miner, specifications in recipe["miner_clusters"].items()
    }
    features = list(
        dict.fromkeys(
            feature
            for specifications in miner_clusters.values()
            for feature, _ in specifications
        )
    )
    print("loading AlphaCrafter prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, workflow_history = alphacrafter_workflow_scores(
        extended,
        miner_clusters,
        common_start=settings["formation_start"],
        validation_horizons=workflow_policy["validation_horizons_months"],
        validation_horizon_weights=workflow_policy["validation_horizon_weights"],
        reward_purge_months=workflow_policy["reward_purge_months"],
        maintenance_history_months=workflow_policy["maintenance_history_months"],
        minimum_maintenance_months=workflow_policy["minimum_maintenance_months"],
        screener_suitability_months=workflow_policy["screener_suitability_months"],
        minimum_absolute_suitability=workflow_policy["minimum_absolute_suitability"],
        maximum_rankic_similarity=workflow_policy["maximum_rankic_similarity"],
        selected_factor_count=workflow_policy["selected_factor_count"],
        minimum_miners_represented=workflow_policy["minimum_miners_represented"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(workflow_history) != len(expected_months):
        raise ValueError("AlphaCrafter reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M053_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete AlphaCrafter path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV AlphaCrafter return path")
    ...
    workflow_history.to_csv(output / "workflow_history.csv", index=False)
    ...
    regime_counts = workflow_history["market_regime"].value_counts().to_dict()
    unique_candidates = len({name for value in workflow_history["selected_candidates"] for name in value.split("|")})
    verdict = f"""# M053: AlphaCrafter in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not the native AlphaCrafter LLM factor/trader path.

The reconstruction preserves three miner clusters, a maintained 48-factor library, purged multi-horizon validation, recent-RankIC suitability, diversity filtering, regime-aware screening, normalized directional weights, and a top-K long-short trader. Five factors are reselected monthly from at least two miners; {unique_candidates} distinct candidates entered the ensemble, and regime counts were {regime_counts}.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen AlphaCrafter recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "miner_count": len(miner_clusters),
        "candidate_count": workflow_policy["total_candidate_count"],
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M053_alphacrafter").resolve()
