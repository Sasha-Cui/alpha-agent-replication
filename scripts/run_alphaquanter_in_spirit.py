#!/usr/bin/env python3
"""Run the frozen M005 AlphaQuanter-inspired selective-tool policy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M005 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("AlphaQuanter recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("AlphaQuanter result must remain explicitly in-spirit")
    policy = recipe["chronological_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("AlphaQuanter recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["reward_conformance_path"]: recipe["strict_evidence"]["reward_conformance_sha256"],
        recipe["strict_evidence"]["dataset_inventory_path"]: recipe["strict_evidence"]["dataset_inventory_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned AlphaQuanter evidence changed: {relative}")
    ...
    features = sorted({name for names in recipe["tool_definitions"].values() for name in names} - {"ret"})
    print("loading AlphaQuanter prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    tools = alphaquanter_tool_scores(extended)
    reward = multi_horizon_monthly_return(
        extended,
        horizons=tuple(policy["monthly_reward_horizons"]),
        eta=policy["exponential_decay_eta"],
    )
    scores, policy_history = alphaquanter_rolling_scores(
        extended,
        tools,
        reward,
        common_start=settings["formation_start"],
        reward_gap_months=policy["reward_information_gap_months"],
        training_months=policy["training_window_months"],
        ridge_penalty=policy["ridge_penalty"],
        maximum_selected_tools=policy["maximum_selected_tools"],
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("AlphaQuanter reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M005_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete AlphaQuanter path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV AlphaQuanter return path")
    ...
    policy_history.to_csv(output / "tool_policy_history.csv", index=False)
    ...
    tool_counts = pd.concat(
        [policy_history["selected_tool_1"], policy_history["selected_tool_2"]],
        ignore_index=True,
    ).value_counts()
    tool_text = ", ".join(f"{name}={int(count)}" for name, count in tool_counts.items())
    verdict = f"""# M005: AlphaQuanter in-spirit reconstruction on monthly U.S./JKP data
...
At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks. Across two selected tools per month, selection counts are {tool_text}.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen AlphaQuanter recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
        "primary_monthly_returns.csv",
        "metrics.csv",
        "attribution_residuals.csv",
        "tool_policy_history.csv",
        "verdict.md",
    ]
    manifest = {
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M005_alphaquanter").resolve()
