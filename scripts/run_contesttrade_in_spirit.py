#!/usr/bin/env python3
"""Run the frozen M025 ContestTrade-inspired dual contest on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M025 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("ContestTrade recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("ContestTrade result must remain explicitly in-spirit")
    contest = recipe["contest_policy"]
    if contest["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("ContestTrade recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["config_conformance_path"]: recipe["strict_evidence"]["config_conformance_sha256"],
        recipe["strict_evidence"]["entrypoint_reachability_path"]: recipe["strict_evidence"]["entrypoint_reachability_sha256"],
        recipe["strict_evidence"]["zi_semantics_path"]: recipe["strict_evidence"]["zi_semantics_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned ContestTrade evidence changed: {relative}")
    paper_path = root / recipe["paper_source"]["pdf_path"]
    if digest(paper_path) != recipe["paper_source"]["pdf_sha256"]:
        raise ValueError("pinned ContestTrade paper identity changed")
    ...
    extended_settings["formation_start"] = contest["prehistory_formation_start"]
    data_agents = [
        (item["column"], int(item["sign"]))
        for item in recipe["data_agents"]
    ]
    research_agents = recipe["research_agents"]
    features = list(
        dict.fromkeys(
            [feature for feature, _ in data_agents]
            + [
                item["column"]
    ...
    print("loading ContestTrade prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, contest_history = contesttrade_dual_contest_scores(
        extended,
        data_agents,
        research_agents,
        common_start=settings["formation_start"],
        data_history_months=contest["data_history_months"],
        data_recent_trend_months=contest["data_recent_trend_months"],
        data_context_budget_agents=contest["data_context_budget_agents"],
        research_context_weight=contest["research_context_weight"],
        research_belief_weight=contest["research_belief_weight"],
        research_history_months=contest["research_history_months"],
        qualitative_judge_weight=contest["qualitative_judge_weight"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(contest_history) != len(expected_months):
        raise ValueError("ContestTrade reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M025_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete ContestTrade path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV ContestTrade return path")
    ...
    contest_history.to_csv(output / "contest_history.csv", index=False)
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen ContestTrade recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "data_agent_count": len(data_agents),
        "research_agent_count": len(research_agents),
        "policy_update_months": len(contest_history),
        "mean_data_selected_count": mean_data_selected,
        "data_no_positive_fallback_months": data_fallback_months,
        "research_no_positive_fallback_months": research_fallback_months,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M025_contesttrade").resolve()
