#!/usr/bin/env python3
"""Run the frozen M020 MASS-inspired heterogeneous-agent simulation on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M020 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("MASS recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("MASS result must remain explicitly in-spirit")
    policy = recipe["simulation_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("MASS recipe permits final-result selection")
    ...
    features = recipe["investor_type_features"]
    print("loading MASS prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, policy_history = mass_simulation_scores(
        extended,
        features,
        ...
        selections_per_agent=policy["selections_per_agent"],
        aggregation_alpha=policy["aggregation_alpha"],
        annealing_initial_temperature=policy["annealing_initial_temperature"],
        annealing_iterations=policy["annealing_iterations"],
        annealing_cooling=policy["annealing_cooling"],
        objective_scale=policy["objective_scale"],
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("MASS reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M020_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete MASS path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV MASS return path")
    ...
    policy_history.to_csv(output / "simulation_history.csv", index=False)
    ...
    average_accepted = float(policy_history["accepted_proposals"].mean())
    verdict = f"""# M020: MASS in-spirit reconstruction on monthly U.S./JKP data
...
At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks. Mean accepted annealing proposals are {average_accepted:.1f} of 100.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen MASS recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M020_mass").resolve()
