#!/usr/bin/env python3
"""Run the frozen M036 FactFin-inspired counterfactual MCTS on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M036 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("FactFin recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("FactFin result must remain explicitly in-spirit")
    policy = recipe["search_and_counterfactual_policy"]
    if policy["final_common_returns_used_for_search_or_selection"] is not False:
        raise ValueError("FactFin recipe permits final-result selection")
    ...
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned FactFin evidence changed: {relative}")
    provenance = json.loads((root / recipe["strict_evidence"]["source_provenance_path"]).read_text())
    if provenance["official_pdf_sha256"] != recipe["paper_source"]["pdf_sha256"]:
        raise ValueError("pinned FactFin paper identity changed")
    if provenance["arxiv_source_sha256"] != recipe["paper_source"]["source_sha256"]:
        raise ValueError("pinned FactFin paper source changed")
    ...
    rag_state = {
        group: [(item["column"], int(item["sign"])) for item in specifications]
        for group, specifications in recipe["rag_state"].items()
    }
    features = list(
        dict.fromkeys(
            feature
            for specifications in rag_state.values()
            for feature, _ in specifications
        )
    )
    print("loading FactFin prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    grammar = recipe["strategy_code_grammar"]
    scores, search_history, search_summary = factfin_counterfactual_mcts_scores(
        extended,
        rag_state,
        common_start=settings["formation_start"],
        training_start=policy["search_training_start"],
        training_end=policy["search_training_end"],
        validation_start=policy["search_validation_start"],
        validation_end=policy["search_validation_end"],
        initial_weights=grammar["initial_weights"],
        mutation_step=grammar["weight_mutation_step"],
        weight_bounds=tuple(grammar["weight_bounds"]),
        hold_band=tuple(grammar["hold_band"]),
        mcts_depth=policy["mcts_depth"],
        ucb_exploration_constant=policy["mcts_ucb_exploration_constant"],
        mcts_iterations=policy["mcts_iterations"],
        counterfactual_finalists=policy["counterfactual_finalists"],
        counterfactual_scenarios=policy["counterfactual_scenarios"],
        counterfactual_seed=policy["counterfactual_seed"],
        final_objective_weights=policy["final_objective_weights"],
    )
    ...
    if (
        ...
        raise ValueError("FactFin reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M036_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete FactFin path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV FactFin return path")
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen FactFin recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "component_count": 4,
        "mcts_evaluations": len(search_history),
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M036_factfin").resolve()
