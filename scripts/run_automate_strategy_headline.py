#!/usr/bin/env python3
"""Evaluate the first selected automated-strategy factor on U.S./JKP."""
...
CANDIDATE_ID = "automate_strategy_price_momentum_14"
SOURCE_FORMULA = "PriceMomentum = CLOSE - DELAY(CLOSE, 14)"
...
def price_momentum_14(history: pd.DataFrame) -> pd.DataFrame:
    """Preserve the literal close-minus-fourteen-bar-close formula."""
    ...
    lagged = frame.groupby("security_id", sort=False)["close"].shift(14)
    frame["score"] = frame["close"] - lagged
    return frame[["security_id", "month", "score"]]


def load_score_history(source_path: Path, start: str, end: str) -> pd.DataFrame:
    start_month = (pd.Timestamp(start) - pd.offsets.MonthEnd(18)).date()
    end_month = pd.Timestamp(end).date()
    raw = pd.read_parquet(
        source_path, columns=["id", "eom", "prc"],
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M069 run already exists; do not silently overwrite it")
    ...
    if contract["status"] != "frozen" or recipe["candidate_id"] != CANDIDATE_ID:
        raise ValueError("frozen strategy-finding recipe/contract mismatch")
    if recipe["source_formula"] != SOURCE_FORMULA or recipe["source_seed_index"] != 1:
        raise ValueError("source-selected first alpha changed")
    if recipe["source_signed_ic"] <= 0:
        raise ValueError("source-signed component direction changed")
    ...
    evidence = {
        ...
        recipe["paper_audit"]["table_3_conformance_path"]: recipe["paper_audit"]["table_3_conformance_sha256"],
    }
    for relative, expected in evidence.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned strategy-finding evidence changed: {relative}")
    implementation = [
    ...
    scores = price_momentum_14(history)
    ...
    holdings_path = private_dir / "M069_formation_holdings.parquet"
    ...
    if not np.isfinite(y).all() or (y <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV strategy-finding path")
    ...
    verdict = f'''# M069: automated strategy finding PriceMomentum on monthly U.S./JKP data
...
'''
    ...
    manifest = {
        "status": "evaluated_partial", "milestone_id": "M069", "candidate_id": CANDIDATE_ID,
        ...
        "primary_result": primary, "source_seed_index": 1, "source_signed_ic": recipe["source_signed_ic"],
        "source_direction": "positive_signed_ic", "prior_jkp_outcomes_seen": True, "confirmatory_claim": False,
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("paper_runs/us_jkp_headline/M069_strategy_finding"))
    ...
    with (private_dir / "operation.lock").open("a+") as lock:
        ...
        status = {
            "state": "running", "phase": "run", "milestone_id": "M069", "pid": os.getpid(),
