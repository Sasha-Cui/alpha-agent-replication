#!/usr/bin/env python3
"""Run the frozen M027 AlphaAgents-inspired round-robin debate on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M027 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("AlphaAgents recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("AlphaAgents result must remain explicitly in-spirit")
    debate = recipe["debate_policy"]
    if debate["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("AlphaAgents recipe permits final-result selection")
    if debate["paper_2024_ticker_memberships_used_as_inputs"] is not False:
        raise ValueError("AlphaAgents recipe permits backcasting paper-selected tickers")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["portfolio_inventory_path"]: recipe["strict_evidence"]["portfolio_inventory_sha256"],
        recipe["strict_evidence"]["method_audit_path"]: recipe["strict_evidence"]["method_audit_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned AlphaAgents evidence changed: {relative}")
    paper_path = root / recipe["paper_source"]["pdf_path"]
    if digest(paper_path) != recipe["paper_source"]["pdf_sha256"]:
        raise ValueError("pinned AlphaAgents paper identity changed")
    ...
    specialists = {
        specialist: [(item["column"], int(item["sign"])) for item in specifications]
        for specialist, specifications in recipe["specialist_agents"].items()
    }
    features = list(
        dict.fromkeys(
            feature
    ...
    print("loading AlphaAgents common formations", flush=True)
    formed = load_formations(source_path, features, settings)
    scores, debate_history = alphaagents_debate_scores(
        formed,
        specialists,
        speaker_order=debate["speaker_order"],
        round_robin_passes=debate["round_robin_passes"],
        peer_median_update_weight=debate["peer_median_update_weight"],
        own_specialist_retention_weight=debate["own_specialist_retention_weight"],
    )
    expected_months = pd.date_range(settings["formation_start"], settings["formation_end"], freq="ME")
    if formed["month"].nunique() != len(expected_months) or len(debate_history) != len(expected_months):
        raise ValueError("AlphaAgents reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M027_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete AlphaAgents path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV AlphaAgents return path")
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen AlphaAgents recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "round_robin_passes": debate["round_robin_passes"],
        "policy_update_months": len(debate_history),
        "mean_initial_disagreement_rate": initial_disagreement,
        "mean_final_disagreement_rate": final_disagreement,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M027_alphaagents").resolve()
