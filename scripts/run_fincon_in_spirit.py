#!/usr/bin/env python3
"""Run the frozen M011 FinCon-inspired manager/analyst policy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M011 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("FinCon recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("FinCon result must remain explicitly in-spirit")
    policy = recipe["chronological_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("FinCon recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["gaps_path"]: recipe["strict_evidence"]["gaps_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned FinCon evidence changed: {relative}")
    ...
    scores, policy_history = fincon_rolling_scores(
        extended,
        analysts,
        common_start=settings["formation_start"],
        procedural_memory_months=recipe["belief_policy"]["procedural_memory_months"],
        minimum_rankic_months=recipe["belief_policy"]["minimum_rankic_months"],
        belief_learning_rate=recipe["belief_policy"]["belief_learning_rate"],
        cvar_history_months=recipe["risk_control"]["return_history_months"],
        cvar_tail_probability=recipe["risk_control"]["monthly_cvar_tail_probability"],
        cvar_penalty=recipe["risk_control"]["cvar_penalty"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("FinCon reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M011_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete FinCon path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV FinCon return path")
    ...
    policy_history.to_csv(output / "belief_history.csv", index=False)
    ...
    final_beliefs = {
        name: float(policy_history[f"belief_weight__{name}"].iloc[-1])
        for name in analysts
    }
    belief_text = ", ".join(f"{name}={value:.3f}" for name, value in final_beliefs.items())
    verdict = f"""# M011: FinCon in-spirit reconstruction on monthly U.S./JKP data
...
At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks. Final numerical beliefs are {belief_text}.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen FinCon recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "final_belief_weights": final_beliefs,
        "average_cvar_alerts": float(policy_history["cvar_alerts"].mean()),
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M011_fincon").resolve()
