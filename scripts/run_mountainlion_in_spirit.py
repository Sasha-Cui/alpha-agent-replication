#!/usr/bin/env python3
"""Run the frozen M024 MountainLion-inspired adaptive fusion on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M024 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("MountainLion recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("MountainLion result must remain explicitly in-spirit")
    fusion = recipe["dual_track_policy"]
    if fusion["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("MountainLion recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["mechanism_conformance_path"]: recipe["strict_evidence"]["mechanism_conformance_sha256"],
        recipe["strict_evidence"]["formula_checks_path"]: recipe["strict_evidence"]["formula_checks_sha256"],
        recipe["strict_evidence"]["source_execution_path"]: recipe["strict_evidence"]["source_execution_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned MountainLion evidence changed: {relative}")
    paper_path = root / recipe["paper_source"]["pdf_path"]
    if digest(paper_path) != recipe["paper_source"]["pdf_sha256"]:
        raise ValueError("pinned MountainLion paper identity changed")
    ...
    extended_settings["formation_start"] = fusion["prehistory_formation_start"]
    modalities = {
        modality: [(item["column"], int(item["sign"])) for item in specifications]
        for modality, specifications in recipe["modalities"].items()
    }
    features = list(
        dict.fromkeys(
            feature
    ...
    print("loading MountainLion prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, fusion_history = mountainlion_fusion_scores(
        extended,
        modalities,
        fusion["ml_features"],
        common_start=settings["formation_start"],
        ml_training_months=fusion["ml_training_months"],
        fusion_history_months=fusion["fusion_history_months"],
        minimum_fusion_rankic_months=fusion["minimum_fusion_rankic_months"],
        ridge_lambda=fusion["ridge_lambda"],
        fusion_temperature=fusion["fusion_temperature"],
        alpha_floor=fusion["alpha_floor"],
        alpha_ceiling=fusion["alpha_ceiling"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(fusion_history) != len(expected_months):
        raise ValueError("MountainLion reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M024_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete MountainLion path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV MountainLion return path")
    ...
    fusion_history.to_csv(output / "fusion_history.csv", index=False)
    ...
    average_alpha = float(fusion_history["llm_alpha"].mean())
    minimum_alpha = float(fusion_history["llm_alpha"].min())
    maximum_alpha = float(fusion_history["llm_alpha"].max())
    verdict = f"""# M024: MountainLion in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not a native MountainLion crypto replication.

The reconstruction preserves four specialized roles: technical analysis, market dynamics, trading-recommendation integration, and semantic reflection. Its deterministic reflection track fuses four JKP modalities and shrinks conviction when their signs conflict. In parallel, a monthly ridge track is refitted on the preceding 60 months of technical ranks. A convex alpha then combines the tracks using only their preceding 24 months of RankIC and directional accuracy. Mean LLM-track alpha was {average_alpha:.3f}, ranging from {minimum_alpha:.3f} to {maximum_alpha:.3f}.
...
This result answers how one transparent MountainLion-inspired multi-agent, dual-track fusion transfers to monthly U.S. stocks. It does not reproduce the paper's crypto inputs, LLM calls, price forecasts, recommendation reports, or native claims.
"""
    (output / "verdict.md").write_text(verdict)
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen MountainLion recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "agent_count": len(recipe["four_specialized_agents"]),
        "modality_count": len(modalities),
        "policy_update_months": len(fusion_history),
        "average_llm_alpha": average_alpha,
        "minimum_llm_alpha": minimum_alpha,
        "maximum_llm_alpha": maximum_alpha,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M024_mountainlion").resolve()
