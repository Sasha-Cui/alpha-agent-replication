#!/usr/bin/env python3
"""Run the frozen M004 FLAG-TRADER-inspired actor/critic on U.S./JKP."""
...
from alpha_evolve.in_spirit import flag_trader_rolling_scores
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M004 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("FLAG-TRADER recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("FLAG-TRADER result must remain explicitly in-spirit")
    policy = recipe["chronological_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("FLAG-TRADER recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["hyperparameters_path"]: recipe["strict_evidence"]["hyperparameters_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned FLAG-TRADER evidence changed: {relative}")
    ...
    features = [item["column"] for item in recipe["state_features"]]
    semantic_prior = [item["semantic_prior_weight"] for item in recipe["state_features"]]
    print("loading FLAG-TRADER prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, policy_history = flag_trader_rolling_scores(
        extended,
        features,
        semantic_prior,
        common_start=settings["formation_start"],
        replay_months=policy["replay_window_months"],
        minimum_training_months=policy["minimum_training_months"],
        ridge_penalty=policy["ridge_penalty"],
        learning_rate=policy["learning_rate"],
        clip_coefficient=policy["clip_coefficient"],
        maximum_gradient_norm=policy["maximum_gradient_norm"],
        action_memory_weight=policy["action_memory_weight"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("FLAG-TRADER reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M004_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete FLAG-TRADER path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV FLAG-TRADER return path")
    ...
    final_weights = {
        feature: float(policy_history[f"actor_weight__{feature}"].iloc[-1])
        for feature in features
    }
    weight_text = ", ".join(f"{name}={value:.3f}" for name, value in final_weights.items())
    verdict = f"""# M004: FLAG-TRADER in-spirit reconstruction on monthly U.S./JKP data
...
At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks. Final actor weights are {weight_text}.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen FLAG-TRADER recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "final_actor_weights": final_weights,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M004_flag_trader").resolve()
