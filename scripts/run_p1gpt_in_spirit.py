#!/usr/bin/env python3
"""Run the frozen M038 P1GPT-inspired layered workflow on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M038 in-spirit run already exists; do not silently overwrite it")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["public_component_path"]: recipe["strict_evidence"]["public_component_sha256"],
        recipe["strict_evidence"]["result_recovery_path"]: recipe["strict_evidence"]["result_recovery_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned P1GPT evidence changed: {relative}")
    paper_path = root / recipe["paper_source"]["pdf_path"]
    if digest(paper_path) != recipe["paper_source"]["pdf_sha256"]:
        raise ValueError("pinned P1GPT paper identity changed")
    ...
    features = list(
        dict.fromkeys(
            [
                feature
                for specifications in domain_agents.values()
                for feature, _ in specifications
            ]
            + [feature for feature, _ in risk_features]
        )
    )
    print("loading P1GPT common formations", flush=True)
    formed = load_formations(source_path, features, settings)
    scores, workflow_history = p1gpt_layered_workflow_scores(
        formed,
        domain_agents,
        risk_features,
    ...
    if formed["month"].nunique() != len(expected_months) or len(workflow_history) != len(expected_months):
        raise ValueError("P1GPT reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M038_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete P1GPT path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV P1GPT return path")
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen P1GPT recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M038_p1gpt").resolve()
