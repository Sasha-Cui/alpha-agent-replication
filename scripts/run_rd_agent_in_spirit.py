#!/usr/bin/env python3
"""Run the frozen M022 R&D-Agent-inspired formula search on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M022 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("R&D-Agent recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("R&D-Agent result must remain explicitly in-spirit")
    search = recipe["research_development_policy"]
    if search["final_common_returns_used_for_search_or_selection"] is not False:
        raise ValueError("R&D-Agent recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["native_execution_path"]: recipe["strict_evidence"]["native_execution_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned R&D-Agent evidence changed: {relative}")
    audit_manifest = json.loads((root / recipe["strict_evidence"]["audit_manifest_path"]).read_text())
    if (
        audit_manifest["paper_hashes"]["arxiv_v2_pdf"]
        != recipe["paper_source"]["pdf_sha256_from_pinned_audit"]
    ):
        raise ValueError("pinned R&D-Agent paper identity changed")
    ...
    extended_settings["formation_start"] = search["prehistory_formation_start"]
    branches = {
        branch: [(item["column"], int(item["sign"])) for item in specifications]
        for branch, specifications in recipe["hypothesis_branches"].items()
    }
    features = list(
        dict.fromkeys(
            feature
    ...
    print("loading R&D-Agent prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, search_history = rd_agent_search_scores(
        extended,
        branches,
        common_start=settings["formation_start"],
        history_months=search["history_months"],
        research_months=search["research_months"],
        validation_months=search["validation_months"],
        minimum_rankic_months=search["minimum_rankic_months"],
        validation_folds=search["validation_folds"],
        validation_fold_months=search["validation_fold_months"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(search_history) != len(expected_months):
        raise ValueError("R&D-Agent reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M022_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete R&D-Agent path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV R&D-Agent return path")
    ...
    merged_months = int(search_history["selected_solution"].str.startswith("merge__").sum())
    selected_counts = {
        str(name): int(count)
        for name, count in search_history["selected_solution"].value_counts().sort_index().items()
    }
    verdict = f"""# M022: R&D-Agent in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not a native R&D-Agent or R&D-Agent-Quant replication.

The reconstruction instantiates the general paper's two-phase data-science workflow as a monthly formula-search task. Six semantic branches each explore four fixed candidate formulas on a 96-month research split; branch winners then enter a standardized 24-month, three-fold aggregated validation layer alongside all-branch and top-three mergers. The selected solution is applied only after its complete 120-month history, and it selected a merged finalist in {merged_months} of 305 formation months. LLM-generated hypotheses, code synthesis, debugging traces, Kaggle tasks, and the paper's MLE-Bench execution are not reproduced.
...
This result answers how one transparent R&D-Agent-inspired search procedure transfers to monthly U.S. stocks. It does not reproduce or validate the paper's MLE-Bench medal claims, and it must not be attributed to the separate R&D-Agent-Quant system.
"""
    (output / "verdict.md").write_text(verdict)
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen R&D-Agent recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "candidate_count": len(branches) * len(search["branch_candidate_operators"]),
        "policy_update_months": len(search_history),
        "merged_solution_months": merged_months,
        "selected_solution_counts": selected_counts,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M022_rd_agent").resolve()
