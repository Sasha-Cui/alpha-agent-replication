#!/usr/bin/env python3
"""Evaluate AlphaSchema's complete example factor on the U.S./JKP contract."""
...
CANDIDATE_ID = "alphaschema_effort_result_reference_cooldown_memory_p20"
SOURCE_PERIOD = 20
...
def common_size_universe(raw: pd.DataFrame, top_n: int) -> pd.DataFrame:
    """Apply the common formation-size rule to score-history months."""
    frame = raw.rename(columns={"id": "security_id", "eom": "month", "me": "weight"}).copy()
    frame["month"] = pd.to_datetime(frame["month"]) + pd.offsets.MonthEnd(0)
    frame["weight"] = pd.to_numeric(frame["weight"], errors="coerce")
    frame = frame.dropna(subset=["security_id", "month", "weight"])
    frame = frame.loc[frame["weight"] > 0].sort_values(["security_id", "month"], kind="mergesort")
    rank = frame.groupby("month", sort=False)["weight"].rank(method="first", ascending=False)
    return frame.loc[rank <= top_n].sort_values(["security_id", "month"], kind="mergesort").reset_index(drop=True)


def alphaschema_example_factor(history: pd.DataFrame, period: int = SOURCE_PERIOD) -> pd.DataFrame:
    """Port the appendix program to a long dynamic-universe monthly panel."""
    ...
    frame["volume"] = pd.to_numeric(frame["tvol"], errors="coerce").fillna(0.0)
    ...
    price_change = frame.groupby("security_id", sort=False)["close"].pct_change(fill_method=None).fillna(0.0)
    volume_change = frame.groupby("security_id", sort=False)["volume"].pct_change(fill_method=None).fillna(0.0)
    synergy = price_change * volume_change
    event_mean = synergy.groupby(frame["month"], sort=False).transform("mean")
    event_std = synergy.groupby(frame["month"], sort=False).transform("std").replace(0.0, 1.0)
    event_z = ((synergy - event_mean) / event_std).fillna(0.0)
    sma = frame.groupby("security_id", sort=False)["close"].transform(
        lambda x: x.rolling(period, min_periods=1).mean()
    )
    distance = ((frame["close"] - sma).abs() / sma.replace(0.0, np.nan)).fillna(0.0)
    distance_mean = distance.groupby(frame["month"], sort=False).transform("mean")
    distance_std = distance.groupby(frame["month"], sort=False).transform("std").replace(0.0, 1.0)
    context_z = ((distance - distance_mean) / distance_std).fillna(0.0)
    combined = event_z + 0.5 * context_z
    ewm_combined = combined.groupby(keys, sort=False).transform(
        lambda x: x.ewm(span=period, adjust=False).mean()
    )
    cooling_factor = 1.0 / (1.0 + ewm_combined.abs() * 0.3)
    cooled = combined * cooling_factor
    direction = np.sign(synergy).replace(0.0, 0.01)
    interim = cooled.abs() * direction
    signal = interim.groupby(keys, sort=False).transform(
        lambda x: x.ewm(span=period, adjust=False).mean()
    )
    frame["score"] = signal.fillna(0.0).replace([np.inf, -np.inf], 0.0).clip(-5.0, 5.0)
    return frame[["security_id", "month", "score"]]


def load_score_history(source_path: Path, settings: dict) -> pd.DataFrame:
    start_month = (pd.Timestamp(settings["formation_start"]) - pd.offsets.MonthEnd(60)).date()
    end_month = pd.Timestamp(settings["formation_end"]).date()
    raw = pd.read_parquet(
        source_path,
        columns=["id", "eom", "me", "prc", "tvol"],
        filters=[("eom", ">=", date.fromisoformat(str(start_month))), ("eom", "<=", date.fromisoformat(str(end_month)))],
    )
    return common_size_universe(raw, settings["top_n_by_formation_market_equity"])


def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M064 run already exists; do not silently overwrite it")
    ...
    if contract["status"] != "frozen" or recipe["candidate_id"] != CANDIDATE_ID:
        raise ValueError("frozen AlphaSchema recipe/contract mismatch")
    if recipe["source_period"] != SOURCE_PERIOD or not recipe["source_direction"].startswith("positive"):
        raise ValueError("source-selected period or direction changed")
    ...
    evidence = {recipe["paper_audit"]["manifest_path"]: recipe["paper_audit"]["manifest_sha256"]}
    for relative, expected in evidence.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned AlphaSchema evidence changed: {relative}")
    implementation = [
        Path(__file__).resolve(), root / "src/alpha_evolve/headline_backtest.py",
    ...
    history = load_score_history(source_path, settings)
    scores = alphaschema_example_factor(history)
    scores = scores.loc[scores.month.between(pd.Timestamp(settings["formation_start"]), pd.Timestamp(settings["formation_end"]))]
    ...
    holdings_path = private_dir / "M064_formation_holdings.parquet"
    ...
    if not np.isfinite(y).all() or (y <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV AlphaSchema path")
    ...
    verdict = f'''# M064: AlphaSchema example factor on monthly U.S./JKP data
...
At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}; minimum signal coverage is {primary['minimum_finite_signal_count']} stocks.
...
'''
    ...
    manifest = {
        "status": "evaluated_partial", "milestone_id": "M064", "candidate_id": CANDIDATE_ID,
        "completed_at_utc": datetime.now(timezone.utc).isoformat(), "benchmark_id": contract["benchmark_id"],
        "code_commit": code_commit, "contract_sha256": frozen_hashes["contract"], "recipe_sha256": frozen_hashes["recipe"],
        "input_sha256": digest(source_path), "benchmark_sha256": digest(factor_path),
        ...
        "primary_result": primary, "source_period": SOURCE_PERIOD, "source_direction": "positive_signed_test_rank_ic",
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("paper_runs/us_jkp_headline/M064_alphaschema"))
    ...
    with (private_dir / "operation.lock").open("a+") as lock:
        ...
        status = {
            "state": "running", "phase": "run", "milestone_id": "M064", "pid": os.getpid(),
