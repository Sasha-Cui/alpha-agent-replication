#!/usr/bin/env python3
"""Run the frozen M014 TradingAgents-inspired debate graph on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M014 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("TradingAgents recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("TradingAgents result must remain explicitly in-spirit")
    policy = recipe["debate_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("TradingAgents recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["native_component_path"]: recipe["strict_evidence"]["native_component_sha256"],
        recipe["strict_evidence"]["mechanism_path"]: recipe["strict_evidence"]["mechanism_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned TradingAgents evidence changed: {relative}")
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("TradingAgents reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M014_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete TradingAgents path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV TradingAgents return path")
    ...
    policy_history.to_csv(output / "debate_history.csv", index=False)
    ...
    risk_counts = policy_history["risk_choice"].value_counts()
    risk_text = ", ".join(f"{name}={int(count)}" for name, count in risk_counts.items())
    verdict = f"""# M014: TradingAgents in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not a native TradingAgents replication.

The reconstruction preserves four market, social, news, and fundamental analysts; explicit bullish and bearish research cases; trader synthesis; risk-seeking, neutral, and conservative proposals; final manager selection; and outcome reflection. Numeric JKP reports replace unavailable tool/LLM text. Analyst reliability comes only from the preceding 60 RankIC observations, while current formation-time market volatility selects the risk view. The released categorical graph and unavailable paper execution are not reproduced.

At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks. Manager risk choices are {risk_text}.
...
This result answers how one transparent TradingAgents-inspired debate graph transfers to the common task. It does not reproduce or validate the paper's LLM trajectories, actions, execution, or 77 table results.
"""
    (output / "verdict.md").write_text(verdict)
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen TradingAgents recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "risk_choice_counts": {str(name): int(count) for name, count in risk_counts.items()},
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M014_tradingagents").resolve()
