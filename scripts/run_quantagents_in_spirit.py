#!/usr/bin/env python3
"""Run the frozen M034 QuantAgents-inspired meeting policy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M034 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("QuantAgents recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("QuantAgents result must remain explicitly in-spirit")
    policy = recipe["memory_and_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("QuantAgents recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["prompt_inventory_path"]: recipe["strict_evidence"]["prompt_inventory_sha256"],
        recipe["strict_evidence"]["site_documentation_path"]: recipe["strict_evidence"]["site_documentation_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned QuantAgents evidence changed: {relative}")
    paper_path = root / recipe["paper_source"]["pdf_path"]
    if digest(paper_path) != recipe["paper_source"]["pdf_sha256"]:
        raise ValueError("pinned QuantAgents paper identity changed")
    ...
    strategy_pool = {
        ...
        for strategy, specifications in recipe["strategy_pool"].items()
    }
    features = list(
        dict.fromkeys(
            feature
            for specifications in strategy_pool.values()
            for feature, _ in specifications
        )
    )
    print("loading QuantAgents prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, meeting_history = quantagents_meeting_scores(
        extended,
        strategy_pool,
        common_start=settings["formation_start"],
        memory_history_months=policy["memory_history_months"],
        retrieved_similar_cases=policy["retrieved_similar_cases"],
        new_strategy_members=policy["new_strategy_members"],
        market_report_weight=policy["market_report_weight"],
        strategy_policy_weight=policy["strategy_policy_weight"],
        adaptive_reward_window=policy["adaptive_reward_window"],
        risk_component_weights=tuple(policy["risk_component_weights"]),
        risk_alert_threshold=policy["risk_alert_threshold"],
        risk_policy_weight_when_triggered=policy["risk_policy_weight_when_triggered"],
    )
    ...
    if (
        formed["month"].nunique() != len(expected_months)
        or len(meeting_history) != len(expected_months)
    ):
        raise ValueError("QuantAgents reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M034_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete QuantAgents path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV QuantAgents return path")
    ...
    meeting_history.to_csv(output / "meeting_history.csv", index=False)
    ...
    risk_alert_months = int(meeting_history["risk_alert_triggered"].sum())
    mean_simulated_weight = float(meeting_history["simulated_reward_weight"].mean())
    proposed_counts: dict[str, int] = {name: 0 for name in strategy_pool}
    for members in meeting_history["proposed_strategy_members"]:
        for name in str(members).split("|"):
            proposed_counts[name] += 1
    verdict = f"""# M034: QuantAgents in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not a native GPT-4o QuantAgents replication.

The reconstruction preserves Otto, Bob, Dave, and Emily; market-analysis and strategy-development meetings; three memory types; ten similar-case retrievals; a ten-member strategy pool; simulated strategy testing; adaptive simulated/real reward blending; and the paper's 0.75 risk-alert trigger. Each month Bob proposes three strategies from past similar-state RankIC utility, Otto blends them with the previously deployed set and Emily's report, and Dave contributes a defensive policy when the equal-weight risk score triggers. Mean simulated-reward weight was {mean_simulated_weight:.3f}; risk meetings triggered in {risk_alert_months} of 305 months. Strategy proposal counts were {proposed_counts}. Missing LLM meetings, tools, raw memories, strategy traces, and learned policy parameters are not recreated.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen QuantAgents recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "agent_count": 4,
        "memory_type_count": 3,
        "policy_update_months": len(meeting_history),
        "risk_alert_months": risk_alert_months,
        "mean_simulated_reward_weight": mean_simulated_weight,
        "strategy_proposal_counts": proposed_counts,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M034_quantagents").resolve()
