...
ARTIFACT_NOTES = {
    "SYS-FIN-ANALYST": (
        ...
        "deployment at commit 85ab4781. All five Space commits and 13 historical paths "
        "are audited; a deleted LFS ZIP is recovered but contains only four source files. "
        "All 103 dataset commits, 204 TSLA/BTC LFS payload revisions, and 327 organizer "
        "commits are also verified. Its app, nine prompt constants, lower-bounded "
        ...
        "20 with 293 rows and BTC ends May 21 with 294, contradicting the common May 10 "
        "endpoint printed in the paper. "
        "Four composite long/short hit/PnL cells recover their hit-rate component but not "
    ...
    "SYS-ALPHA-AGENT": (
        ...
        "Qlib's native MLflowRecorder also loads all seven records twice with zero network "
        "attempts: 133 metrics, 189 params, 35 tags, and 21 config/dataset/task artifacts "
        "resolve; a non-unpickling loader verifies every artifact hash and all 35 relevant "
        "scalar values against the fail-closed parser. Every multi-record mean/median "
        "hypothesis is enumerated: 30 candidates in total. "
        ...
        "Table 2 cells. This is native Qlib-recorder author-artifact corroboration, not "
        "regeneration, because inputs, predictions, returns, and holdings are absent. "
        "The exact paper-era Qlib downloader falls back to a hash-pinned 450,094,816-byte "
        "US archive whose calendar ends on 2020-11-10 and lacks the configured SPX "
        "benchmark, so it cannot execute the matching run's 2021--2024 test. The latest "
        "public commit at the run timestamp contains four US candidate expressions while "
        "the fitted model requires five generated features; the paper snapshot contains "
        "six only after two more were added. No combined_factors_df.pkl exists anywhere "
        "in the complete public history, so the exact five-factor values and ordering are "
        "also unrecoverable. This replay failure adds no paper-result credit. "
        ...
        "No frozen paper dataset, prediction, holding, return, complete prediction/portfolio "
        "recorder, baseline output, or figure array survives. Thus 5/100 Table 2 cells are corroborated, "
    ...
    "SYS-AUTOMATE-STRATEGY": (
        ...
        "and a seed workbook. The paper claims a 100-alpha SAF across nine categories; "
        "the workbook releases only 37 alphas across seven categories, with 32 rows in "
        "the five Table 2 categories and five unprinted Quality/Technical rows. The "
        "inferable mean-absolute-IC rule reproduces 3/10 Table 2 cells at four-decimal "
        "precision, all category-wide means from this incomplete universe. Exhausting "
        "all 192 same-size selected-alpha subsets finds zero selected-mean value match. "
        "The Table 3 subset is uniquely closest in all five categories, which supports "
        "selection identity but not the displayed IC values. The same workbook "
        "corroborates all 12 signed IC cells for Table 3's selected alphas; its 12 "
        "learned weights and combined IC remain unavailable. This is component evidence, "
        ...
        "node rather than 10 and lacks its inputs; the top-k/drop-n portfolio rule is "
        "absent. The later generic Grail branch is disconnected from the workbooks and "
        "does not implement the paper configuration. A dated census exhausts all 24 "
        "public forks and 25 branch refs: every fork head is an exact commit already in "
        "the official history, adding no result or log lineage. The paper also conflicts "
        "internally on SSE50 return and full-model ablation Sharpe. The factor outputs "
        "are not a six-country security-level signal panel.",
    ),
    "SYS-QUANTA-ALPHA": (
        "Y",
        ...
        "The untouched released factor-coder test fails in a complete 351-line declared-"
        "dependency environment because template_debug.jinjia2 is absent. A copied-tree "
        "audit maps that filename byte-for-byte to the shipped template and derives the "
        "missing $return column from the official 48,700-row HDF with the release's own "
        "instrument-level close-return formula. The original test then passes twice, "
        "producing identical canonical 48,700-point factor outputs (48,600 finite and "
        "100 warm-up NaNs) with zero network attempts. The series has no published target "
        "or experiment lineage and receives zero paper-result credit. "
        ...
        "(0/7 current metrics) and returns -2.32% rather than +4.09%. A bounded "
        "primary-source protocol grid also runs Qlib v0.9.7's exact official "
        ...
        "ARR and IR relative to the prior 168-factor diagnostic, so the pair cannot explain the "
        "headline gap. The unmodified released operator library still fails on those two expressions. Exact model "
    ...
    "SYS-GPT-SIGNAL": (
        ...
        "box statistics preserve the released result's shape under an untraceable "
        "+0.02 published-vector translation, which receives no result credit. The "
        ...
        "commits in the surviving author repository, 230 reachable output/text blobs, and 1,356 candidate "
        "20-value windows finds exact preserved Energy 3M outputs (35/35 vector "
        "statistics), an obsolete IT family (0/35 against the paper), and no direct "
        "or shape-translated all-sector match. GH Archive also identifies a deleted "
        "predecessor repository with 12 one-commit pushes and at least 13 known "
        "commit hashes; neither GitHub nor Software Heritage retains its content. "
        "It predates the recovered EVC output and paper vector and receives no result "
        "credit, but prevents treating the surviving history as the account's complete "
        "public lineage. The only other author-coded EVC "
        "formula misses by up to 0.0959; the sole fork and exact public code searches "
        "add no missing trace. The current runner exits before "
    ...
    "SYS-RAPTOR": (
        ...
        "2%/252 daily RF, and sqrt(252) annualization. All eight standard 20-day "
        "convention combinations miss every one of the four Section 4.3 claims. "
        "Across 1,160 longer-window convention combinations, 290 hit at least one "
        "rounded endpoint and only 11 hit both, so the paper does not identify a "
        "unique longer-window protocol. A hash-pinned current Yahoo S&P 500 response "
        "additionally verifies the two displayed 10.08% benchmark units and the "
        "3.35-point comparison; those three checks are not paper-time input lineage. "
        "Three displayed perturbation deltas recompute exactly from other paper cells, "
        "and five explanation values exactly duplicate table cells, adding 8/42 "
        "paper-internal consistency checks without native-result credit. In total, "
        "29/42 displayed units verify at their declared precision and 36/42 are checked: "
        "five rolling claims conflict, two longer-window endpoint claims are "
        "underspecified, and six unique table weights remain unavailable. This is also "
        "enough to test the rendered figures without inflating raw-series credit. "
        ...
        "author-output/current-response/paper-internal verification, not experiment reproduction: "
        ...
        "while paper/source cadence and view construction conflict. Full four-commit "
        "history inspection finds no missing inputs; the "
    ...
    "SYS-P1GPT": (
        ...
        "GOOGL B&H MDD computes to 6.14%, not the printed 6.41%. Rounding inversion "
        "shows the three P1GPT Sharpe cells jointly admit 251-day annualization while "
        "the nine rule-baseline Sharpe cells jointly admit 252, with no single common "
        "convention. On the same GOOGL close path, matching reported return requires "
        "A P1GPT-only Sharpe truncation rule would display all 12 P1GPT cells as "
        "printed, but no formatter implementation supports it, global truncation "
        "recovers only 26/48 recalculated cells, and the method-specific rule still "
        "recovers only 47/48 overall because GOOGL B&H MDD remains contradictory. "
        ...
        "checks under ordinary rounding verify author outputs and baselines. A separate calibration of the "
    ...
    "SYS-EMPIRICAL-ASSET-PRICING-LLM": (
        ...
        "complete dated public-fork surface has 14 accessible forks, 14 branch refs, "
        "no tags, and four unique heads; every head is an official-history commit, so "
    ...
    "SYS-FIN-MEM": (
        ...
        "receives result credit. The exact historical parameterized main writes all three "
        "ablation tables for all 15 recovered action configurations after only rebinding "
        "get_price and author-local action paths to hash-pinned public inputs. No source "
        "formula changes, no live downloads occur, and all 75 written values agree with "
        "calculate_metrics within 1e-12; 67/75 displayed Table 3--5 cells match the paper. "
        "This is author-output and native metric-main evidence, not end-to-end FinMem "
        "reproduction. A dated census exhausts 181 "
        ...
        "all-hold 2016 TSLA mini-run with the wrong model and top-K; a second fork has "
        "GPT-3.5/top-K=3 checkpoint state but no action or metric result. The original "
        "inputs, memories, complete five-trial lineage, and exact paper configuration "
        "remain absent.",
    ),
    "SYS-HEDGE-AGENTS": (
        "N",
    ...
    "SYS-MACI": (
        ...
        "lineage is still unverified. "
        "An exhaustive scan of 7,262 reachable blobs / 424,295,149 bytes recovers a "
        "fully qualified GPT-4o fine-tuned model ID first committed on 2024-10-26 and "
        "present in the pinned pre-submission source, but only in a comment. An older "
        "GPT-3.5 ID was removed before submission. No ftjob identifier or genuine "
        "OpenAI file identifier survives, and seven exact public searches for the "
        "model suffixes, combined dataset, and checkpoint names return zero matches. "
        "The GPT-4o string is paper-era checkpoint evidence, not proof of active model "
        "selection or paper-run lineage. The paper-listed anonymous v3 artifact resolves to the first "
...
PAPER_ONLY_AUDIT_NOTES = {
    ...
    "SYS-FACTOR-MINER": (
        ...
        "no native result credit is assigned. A second pinned repository created "
        "one day after submission explicitly calls itself paper-inspired and "
        "substitutes Claude, AkShare, and pure NumPy. Its unchanged no-LLM pipeline "
        "fails on an absent hard-coded sibling parquet; a network-blocked synthetic "
        "harness executes its 16 shipped expressions and repeats bit-identically, "
        "which remains candidate-component evidence with zero paper-result credit. "
        "No attributable runtime, A-share or "
    ...
    "SYS-EFS": (
        ...
        "and only 1/12 v2 cells. The six-commit JMLR-linked SSPO source on the five "
        "pinned OLPS datasets reproduces all 10 original-paper CW/SR cells with "
        "bit-identical repeated paths, but matches only 1/15 v1 and 2/12 v2 EFS "
        "cells. Separately, all 18 ASMCVaR configurations run under "
...
TARGETED_EXECUTION = {
    ...
    "SYS-ALPHA-AGENT": (
        "paper_audit:partial_5_of_100_table_cells_corroborated_by_native_qlib_recorder_"
        "7_records_133_metrics_21_artifacts_zero_regenerated_"
        "paper_era_host_and_qlib_environments_112_of_113_modules_"
    ...
    "SYS-AUTOMATE-STRATEGY": (
        ...
        "5_declared_unique_closest_zero_of_40_integrated_portfolio_cells_24_forks_exhausted"
    ),
    "SYS-CONTEST-TRADE": (
    ...
    "SYS-FIN-MEM": "paper_audit:partial_227_of_235_author_output_cells_corroborated_67_of_75_native_metric_main_cells_reproduced_zero_end_to_end_agent_cells_181_accessible_forks_187_refs_20_unique_heads_exhausted",
    ...
    "SYS-QUANTA-ALPHA": (
        ...
        "2_hybrids_0_of_8_predictive_profile_cells_upstream_test_2_of_2_48700_points_"
        "zero_result_credit_270_author_output_units_267_accessible_"
