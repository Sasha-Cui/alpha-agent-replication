#!/usr/bin/env python3
"""Run the frozen M018 HedgeAgents-inspired allocation conference on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M018 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("HedgeAgents recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("HedgeAgents result must remain explicitly in-spirit")
    policy = recipe["conference_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("HedgeAgents recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["profiles_path"]: recipe["strict_evidence"]["profiles_sha256"],
        recipe["strict_evidence"]["method_path"]: recipe["strict_evidence"]["method_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned HedgeAgents evidence changed: {relative}")
    ...
    print("loading HedgeAgents prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, policy_history = hedgeagents_conference_scores(
        extended,
        specialists,
        common_start=settings["formation_start"],
        history_months=policy["history_months"],
        minimum_rankic_months=policy["minimum_rankic_months"],
        cvar_tail_probability=policy["cvar_tail_probability"],
        variance_penalty=policy["variance_penalty"],
        cvar_penalty=policy["cvar_penalty"],
        experience_sharing_weight=policy["experience_sharing_weight"],
        extreme_one_month_threshold=policy["extreme_one_month_threshold"],
        extreme_three_month_threshold=policy["extreme_three_month_threshold"],
        extreme_defensive_minimum_weight=policy["extreme_defensive_minimum_weight"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("HedgeAgents reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M018_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete HedgeAgents path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV HedgeAgents return path")
    ...
    policy_history.to_csv(output / "conference_history.csv", index=False)
    ...
    extreme_months = int(policy_history["extreme_conference"].sum())
    verdict = f"""# M018: HedgeAgents in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not a native HedgeAgents replication.

The reconstruction preserves three speculative, equity, and defensive specialists; a monthly manager conference using expected RankIC, full covariance, and downside-tail utility; cross-specialist experience sharing; and extraordinary defensive reallocation. Crypto and FX agents are represented by U.S. speculative and defensive style sleeves so every score remains in the common universe. All conference statistics use only the preceding 60 months. The private agents, prompts, optimizer, and cross-asset actions are not reproduced.

At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks. The extreme-market conference fired in {extreme_months} months.
...
This result answers how one transparent HedgeAgents-inspired allocation conference transfers to monthly U.S. stocks. It does not reproduce or validate its native cross-asset system or published results.
"""
    (output / "verdict.md").write_text(verdict)
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen HedgeAgents recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
        "primary_monthly_returns.csv",
        "metrics.csv",
        "attribution_residuals.csv",
        "conference_history.csv",
        "verdict.md",
    ]
    manifest = {
        ...
        "extreme_conference_months": extreme_months,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M018_hedgeagents").resolve()
