#!/usr/bin/env python3
"""Run the frozen M040 FINRS-inspired risk-sensitive policy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M040 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("FINRS recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("FINRS result must remain explicitly in-spirit")
    risk_policy = recipe["risk_sensitive_policy"]
    if risk_policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("FINRS recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["cross_paper_lineage_path"]: recipe["strict_evidence"]["cross_paper_lineage_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned FINRS evidence changed: {relative}")
    provenance = json.loads((root / recipe["strict_evidence"]["source_provenance_path"]).read_text())
    if provenance["pinned_input_sha256"]["primary/official-v1.pdf"] != recipe["paper_source"]["pdf_sha256"]:
        raise ValueError("pinned FINRS paper identity changed")
    if provenance["pinned_input_sha256"]["primary/source-v1.tar"] != recipe["paper_source"]["source_sha256"]:
        raise ValueError("pinned FINRS paper source changed")
    ...
    extended_settings["formation_start"] = risk_policy["prehistory_formation_start"]
    signal_memory = {
        layer: [(item["column"], int(item["sign"])) for item in specifications]
        for layer, specifications in recipe["hierarchical_signal_memory"].items()
    }
    features = list(
        dict.fromkeys(feature for specifications in signal_memory.values() for feature, _ in specifications)
    )
    print("loading FINRS prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, risk_history = finrs_risk_sensitive_scores(
        extended,
        signal_memory,
        common_start=settings["formation_start"],
        reward_horizons=risk_policy["monthly_reward_horizons"],
        multi_timescale_weights=risk_policy["multi_timescale_weights"],
        reward_label_purge_months=risk_policy["reward_label_purge_months"],
        reward_history_months=risk_policy["reward_history_months"],
        direction_hold_threshold=risk_policy["direction_hold_threshold"],
        base_trade_quantity=risk_policy["base_trade_quantity"],
        kelly_history_months=risk_policy["kelly_history_months"],
        scaled_kelly_fraction=risk_policy["scaled_kelly_fraction"],
        cvar_history_months=risk_policy["cvar_history_months"],
        cvar_tail_probability=risk_policy["cvar_tail_probability"],
        cvar_risk_budget=risk_policy["cvar_risk_budget"],
        maximum_absolute_exposure=risk_policy["maximum_absolute_exposure"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(risk_history) != len(expected_months):
        raise ValueError("FINRS reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M040_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete FINRS path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV FINRS return path")
    ...
    risk_history.to_csv(output / "risk_history.csv", index=False)
    ...
    mean_base_position = float(risk_history["mean_base_absolute_position"].mean())
    mean_final_exposure = float(risk_history["mean_final_absolute_exposure"].mean())
    mean_scaled_kelly = float(risk_history["mean_scaled_kelly"].mean())
    mean_volatility_adjustment = float(risk_history["mean_volatility_adjustment"].mean())
    total_risk_shrunk = int(risk_history["risk_shrunk_count"].sum())
    total_risk_zeroed = int(risk_history["risk_zeroed_count"].sum())
    verdict = f"""# M040: FINRS in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not a native FINRS replication.

The reconstruction preserves hierarchical market analysis, separate direction and quantity/risk decisions, carried position state, delayed multi-timescale reflection, scaled Kelly sizing, CVaR downside control, volatility adjustment, and an account-exposure ceiling. It first computes the frozen FinPos-inspired base position, using only 60 reward months ending six months earlier, then shrinks rather than reverses that position by the minimum of base magnitude, half Kelly times volatility adjustment, a 5% CVaR cap, and 0.75. Mean absolute exposure fell from {mean_base_position:.3f} to {mean_final_exposure:.3f}; the risk layer shrank {total_risk_shrunk} stock-months and zeroed {total_risk_zeroed}. The exact LLM analyses, native probability estimates, daily account ledger, and integer-share execution are not recreated.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen FINRS recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "memory_layer_count": len(signal_memory),
        "decision_agent_count": 2,
        "policy_update_months": len(risk_history),
        "mean_base_absolute_position": mean_base_position,
        "mean_final_absolute_exposure": mean_final_exposure,
        "mean_scaled_kelly": mean_scaled_kelly,
        "mean_volatility_adjustment": mean_volatility_adjustment,
        "total_risk_shrunk": total_risk_shrunk,
        "total_risk_zeroed": total_risk_zeroed,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M040_finrs").resolve()
