#!/usr/bin/env python3
"""Run the frozen M039 FinPos-inspired carried-position policy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M039 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("FinPos recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("FinPos result must remain explicitly in-spirit")
    position_policy = recipe["position_policy"]
    if position_policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("FinPos recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["prompt_contract_path"]: recipe["strict_evidence"]["prompt_contract_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned FinPos evidence changed: {relative}")
    provenance = json.loads((root / recipe["strict_evidence"]["source_provenance_path"]).read_text())
    if provenance["pinned_input_sha256"]["primary/official-v2.pdf"] != recipe["paper_source"]["pdf_sha256"]:
        raise ValueError("pinned FinPos paper identity changed")
    if provenance["pinned_input_sha256"]["primary/source-v2.tar"] != recipe["paper_source"]["source_sha256"]:
        raise ValueError("pinned FinPos paper source changed")
    ...
    extended_settings["formation_start"] = position_policy["prehistory_formation_start"]
    signal_memory = {
    ...
    features = list(
        dict.fromkeys(
            feature
            for specifications in signal_memory.values()
            for feature, _ in specifications
        )
    )
    print("loading FinPos prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, position_history = finpos_position_scores(
        extended,
        signal_memory,
        common_start=settings["formation_start"],
        reward_horizons=position_policy["monthly_reward_horizons"],
        multi_timescale_weights=position_policy["multi_timescale_weights"],
        reward_label_purge_months=position_policy["reward_label_purge_months"],
        reward_history_months=position_policy["reward_history_months"],
        minimum_reward_months=position_policy["minimum_reward_months"],
        memory_reliability_temperature=position_policy["memory_reliability_temperature"],
        direction_hold_threshold=position_policy["direction_hold_threshold"],
        position_bounds=tuple(position_policy["position_bounds"]),
        base_trade_quantity=position_policy["base_trade_quantity"],
        cvar_history_months=position_policy["cvar_history_months"],
        cvar_tail_probability=position_policy["cvar_tail_probability"],
        cvar_risk_budget=position_policy["cvar_risk_budget"],
        cvar_position_cap_bounds=tuple(position_policy["cvar_position_cap_bounds"]),
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(position_history) != len(expected_months):
        raise ValueError("FinPos reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M039_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete FinPos path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV FinPos return path")
    ...
    position_history.to_csv(output / "position_history.csv", index=False)
    ...
    aggregate_directions = {
        "buy": int(position_history["direction_buy_count"].sum()),
        "hold": int(position_history["direction_hold_count"].sum()),
        "sell": int(position_history["direction_sell_count"].sum()),
    }
    mean_position = float(position_history["mean_absolute_position"].mean())
    mean_quantity = float(position_history["mean_trade_quantity"].mean())
    verdict = f"""# M039: FinPos in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not a native GPT-4o FinPos replication.

The reconstruction preserves shallow news, middle technical, and deep fundamental memory; a separate Direction Agent; a Quantity/Risk Agent; continuous carried positions; delayed multi-timescale reflection; and 95% lower-tail CVaR exposure caps. Every month the Direction Agent reliability-weights memory layers using only 60 reward months ending six months earlier. The Quantity/Risk Agent moves each carried stock position by at most 0.25 times confidence and clips it to a risk-budget/CVaR cap. Aggregate directions were {aggregate_directions}; mean absolute carried position was {mean_position:.3f} and mean trade quantity was {mean_quantity:.3f}. Missing prompts, integer-share conversion, and native account history are not recreated.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen FinPos recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "policy_update_months": len(position_history),
        "aggregate_directions": aggregate_directions,
        "mean_absolute_position": mean_position,
        "mean_trade_quantity": mean_quantity,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M039_finpos").resolve()
