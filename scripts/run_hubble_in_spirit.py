#!/usr/bin/env python3
"""Run the frozen M051 Hubble-inspired safe diverse factor search on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M051 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("Hubble recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("Hubble result must remain explicitly in-spirit")
    search_policy = recipe["safe_search_policy"]
    if search_policy["final_common_returns_used_for_search"] is not False:
        raise ValueError("Hubble recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["model_release_chronology_path"]: recipe["strict_evidence"]["model_release_chronology_sha256"],
        recipe["strict_evidence"]["sandbox_component_execution_path"]: recipe["strict_evidence"]["sandbox_component_execution_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned Hubble evidence changed: {relative}")
    ...
    if audit["attributable_native_implementation_recovered"] is not False:
        raise ValueError("pinned Hubble release status changed")
    if provenance["arxiv"]["id"] != "2604.09601":
        raise ValueError("pinned Hubble paper identity changed")
    ...
    factor_families = {
        family: [(item["column"], int(item["sign"])) for item in specifications]
        for family, specifications in recipe["factor_families"].items()
    }
    features = list(
        dict.fromkeys(
            feature
            for specifications in factor_families.values()
            for feature, _ in specifications
        )
    )
    print("loading Hubble prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, candidate_metrics, selection_history, search_summary = hubble_safe_diverse_scores(
        extended,
        factor_families,
        calibration_start=search_policy["calibration_start"],
        calibration_end=search_policy["calibration_end"],
        common_start=settings["formation_start"],
        mining_rounds=search_policy["mining_rounds"],
        metric_weights=search_policy["metric_weights"],
        negative_rag_crowded_family=search_policy["negative_rag_crowded_family"],
        negative_rag_penalty=search_policy["negative_rag_penalty"],
        novelty_bonus=search_policy["novelty_bonus"],
        maximum_similarity=search_policy["maximum_similarity"],
        family_cap=search_policy["family_cap"],
    ...
    if formed["month"].nunique() != len(expected_months) or scores.notna().sum() != len(formed):
        raise ValueError("Hubble reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M051_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete Hubble path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV Hubble return path")
    ...
    selected_text = "; ".join(search_summary["selected_candidates"])
    family_counts = pd.Series(search_summary["selected_families"]).value_counts().to_dict()
    verdict = f"""# M051: Hubble in-spirit reconstruction on monthly U.S./JKP data
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen Hubble recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "family_count": len(factor_families),
        "mining_rounds": search_policy["mining_rounds"],
        "candidate_count": search_summary["candidate_count"],
        "selection_records": len(selection_history),
        "selected_candidates": search_summary["selected_candidates"],
        "selected_families": search_summary["selected_families"],
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M051_hubble").resolve()
