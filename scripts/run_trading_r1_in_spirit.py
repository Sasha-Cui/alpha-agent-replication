#!/usr/bin/env python3
"""Run the frozen M032 Trading-R1-inspired reward policy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M032 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("Trading-R1 recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("Trading-R1 result must remain explicitly in-spirit")
    policy = recipe["chronological_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("Trading-R1 recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["spec_diagnostics_path"]: recipe["strict_evidence"]["spec_diagnostics_sha256"],
        recipe["strict_evidence"]["mechanism_conformance_path"]: recipe["strict_evidence"]["mechanism_conformance_sha256"],
        recipe["strict_evidence"]["release_inventory_path"]: recipe["strict_evidence"]["release_inventory_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned Trading-R1 evidence changed: {relative}")
    paper_path = root / recipe["paper_source"]["pdf_path"]
    if digest(paper_path) != recipe["paper_source"]["pdf_sha256"]:
        raise ValueError("pinned Trading-R1 paper identity changed")
    ...
    reasoning_groups = {
        ...
        for group, specifications in recipe["reasoning_stages"]["structured_thesis"].items()
    }
    features = list(
        dict.fromkeys(
            feature
            for specifications in reasoning_groups.values()
            for feature, _ in specifications
        )
    )
    print("loading Trading-R1 prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    actions = recipe["five_actions"]
    scores, policy_history = trading_r1_reward_policy_scores(
        extended,
        reasoning_groups,
        action_names=actions["names"],
        action_values=actions["score_values"],
        truth_quantiles=actions["truth_quantiles"],
        reward_matrix=np.asarray(actions["decision_reward_matrix_rows_prediction_columns_truth"], dtype=float),
        common_start=settings["formation_start"],
        label_horizons=policy["monthly_label_horizons"],
        horizon_weights=policy["multi_horizon_weights"],
        volatility_lookback_months=policy["volatility_lookback_months"],
        label_purge_months=policy["label_purge_months"],
        policy_training_months=policy["policy_training_months"],
    ...
    if (
        ...
        or len(policy_history) != len(expected_months)
    ):
        raise ValueError("Trading-R1 reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M032_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete Trading-R1 path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV Trading-R1 return path")
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen Trading-R1 recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "reasoning_stage_count": 3,
        "action_count": len(actions["names"]),
        "policy_update_months": len(policy_history),
        "aggregate_action_counts": action_counts,
        "mean_group_relative_advantage": mean_advantage,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M032_trading_r1").resolve()
