#!/usr/bin/env python3
"""Run the frozen M063 Fin-Analyst specialist hierarchy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M063 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("Fin-Analyst recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("Fin-Analyst result must remain explicitly in-spirit")
    meta_policy = recipe["meta_agent_policy"]
    if meta_policy["paper_or_common_future_returns_used_for_aggregation"] is not False:
        raise ValueError("Fin-Analyst recipe permits future-result aggregation")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["prompt_correspondence_path"]: recipe["strict_evidence"][
            "prompt_correspondence_sha256"
        ],
        recipe["strict_evidence"]["native_execution_path"]: recipe["strict_evidence"]["native_execution_sha256"],
        recipe["strict_evidence"]["organizer_scorer_path"]: recipe["strict_evidence"]["organizer_scorer_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned Fin-Analyst evidence changed: {relative}")
    ...
    if audit["attributable_pre_live_native_implementation_found"] is not True:
        raise ValueError("pinned Fin-Analyst implementation status changed")
    if provenance["arxiv_id"] != "2607.12233":
        raise ValueError("pinned Fin-Analyst paper identity changed")
    if provenance["author_space"]["commit"] != recipe["paper_source"]["author_space_commit"]:
        raise ValueError("pinned Fin-Analyst author release changed")
    ...
    specialist_specs = {
        name: [(item["column"], int(item["sign"])) for item in specifications]
        for name, specifications in recipe["specialists"].items()
    }
    features = list(
        dict.fromkeys(feature for specifications in specialist_specs.values() for feature, _ in specifications)
    )
    print("loading Fin-Analyst common formations", flush=True)
    formed = load_formations(source_path, features, settings)
    scores, specialist_history = fin_analyst_hierarchy_scores(
        formed,
        specialist_specs,
        meta_policy["specialist_weights"],
        specialist_action_threshold=meta_policy["specialist_action_threshold"],
        news_override_confidence=meta_policy["news_override_confidence"],
        news_override_minimum_absolute_catalyst=meta_policy["news_override_minimum_absolute_catalyst"],
        majority_specialists=meta_policy["majority_specialists"],
        split_panel_hold_threshold=meta_policy["split_panel_hold_threshold"],
    )
    expected_months = pd.date_range(settings["formation_start"], settings["formation_end"], freq="ME")
    if formed["month"].nunique() != len(expected_months) or len(specialist_history) != len(expected_months):
        raise ValueError("Fin-Analyst reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M063_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete Fin-Analyst path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV Fin-Analyst return path")
    ...
    specialist_history.to_csv(output / "specialist_history.csv", index=False)
    ...
    resolution_counts = {
        "news_override": int(specialist_history["news_override_count"].sum()),
        "majority_buy": int(specialist_history["majority_buy_count"].sum()),
        "majority_sell": int(specialist_history["majority_sell_count"].sum()),
        "weighted": int(specialist_history["weighted_resolution_count"].sum()),
    }
    meta_action_counts = {
        "buy": int(specialist_history["meta_buy_count"].sum()),
        "hold": int(specialist_history["meta_hold_count"].sum()),
        "sell": int(specialist_history["meta_sell_count"].sum()),
    }
    specialist_action_counts = {
        name: {
            "buy": int(specialist_history[f"{name}_buy_count"].sum()),
            "hold": int(specialist_history[f"{name}_hold_count"].sum()),
            "sell": int(specialist_history[f"{name}_sell_count"].sum()),
        }
        for name in specialist_specs
    }
    verdict = f"""# M063: Fin-Analyst in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not the native Fin-Analyst gpt-4o-mini or TSLA action path.

The reconstruction preserves the paper's TSLA branch: eight source-specific specialists, action/confidence records, a high-confidence news override, five-specialist consensus, recency-weighted resolution, and split-panel HOLD behavior. Across 305,000 security-months, resolution counts were {resolution_counts} and final action counts were {meta_action_counts}. The fixed hierarchy uses no historical paper action or current forward return.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen Fin-Analyst recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
        "primary_monthly_returns.csv",
        "metrics.csv",
        "attribution_residuals.csv",
        "specialist_history.csv",
        "verdict.md",
    ]
    manifest = {
        ...
        "policy_months": len(specialist_history),
        "resolution_counts": resolution_counts,
        "meta_action_counts": meta_action_counts,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M063_fin_analyst").resolve()
