#!/usr/bin/env python3
"""Run the frozen M049 TrustTrade-inspired selective-consensus policy."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M049 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("TrustTrade recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("TrustTrade result must remain explicitly in-spirit")
    memory_policy = recipe["temporal_and_memory_policy"]
    if memory_policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("TrustTrade recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["method_specification_path"]: recipe["strict_evidence"]["method_specification_sha256"],
        recipe["strict_evidence"]["human_interface_protocol_path"]: recipe["strict_evidence"]["human_interface_protocol_sha256"],
        recipe["strict_evidence"]["input_temporal_contamination_path"]: recipe["strict_evidence"]["input_temporal_contamination_sha256"],
        recipe["strict_evidence"]["published_result_panel_ledger_path"]: recipe["strict_evidence"]["published_result_panel_ledger_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned TrustTrade evidence changed: {relative}")
    ...
    if audit["attributable_trusttrade_pipeline_found"] is not False:
        raise ValueError("pinned TrustTrade release status changed")
    if provenance["work_id"] != "CensusArxiv260322567":
        raise ValueError("pinned TrustTrade paper identity changed")
    ...
    extended_settings["formation_start"] = memory_policy["prehistory_formation_start"]
    domain_reports = {
        domain: {
            report: [(item["column"], int(item["sign"])) for item in specifications]
            for report, specifications in reports.items()
        }
        for domain, reports in recipe["independent_domain_reports"].items()
    }
    risk_features = [(item["column"], int(item["sign"])) for item in memory_policy["risk_features"]]
    features = list(
        dict.fromkeys(
            feature
            for reports in domain_reports.values()
            for specifications in reports.values()
            for feature, _ in specifications
        )
    )
    features.extend(feature for feature, _ in risk_features if feature not in features)
    print("loading TrustTrade prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, reflection_history = trusttrade_selective_consensus_scores(
        extended,
        domain_reports,
        risk_features,
        common_start=settings["formation_start"],
        temporal_horizons_months=memory_policy["temporal_horizons_months"],
        reflection_reward_horizons_months=memory_policy["reflection_reward_horizons_months"],
        reflection_reward_weights=memory_policy["reflection_reward_weights"],
        reflection_reward_purge_months=memory_policy["reflection_reward_purge_months"],
        short_reflection_history_months=memory_policy["short_reflection_history_months"],
        long_reflection_history_months=memory_policy["long_reflection_history_months"],
        minimum_long_reflection_months=memory_policy["minimum_long_reflection_months"],
        short_reflection_weight=memory_policy["short_reflection_weight"],
        long_reflection_weight=memory_policy["long_reflection_weight"],
        reliability_temperature=memory_policy["reliability_temperature"],
        semantic_weight_lambda=recipe["selective_consensus"]["semantic_weight_lambda"],
        numeric_scale_sigma=recipe["selective_consensus"]["numeric_scale_sigma"],
        support_weight_alpha=recipe["selective_consensus"]["support_weight_alpha"],
        high_consensus_threshold=recipe["selective_consensus"]["high_consensus_threshold"],
        low_consensus_weight=recipe["selective_consensus"]["low_consensus_weight"],
        consensus_evidence_weight=memory_policy["consensus_evidence_weight"],
        temporal_anchor_weight=memory_policy["temporal_anchor_weight"],
        trader_hold_threshold=memory_policy["trader_hold_threshold"],
        confidence_thresholds=tuple(memory_policy["confidence_thresholds"]),
        position_sizes=tuple(memory_policy["position_sizes"]),
        risk_cap_bounds=(memory_policy["minimum_risk_cap"], memory_policy["maximum_risk_cap"]),
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(reflection_history) != len(expected_months):
        raise ValueError("TrustTrade reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M049_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete TrustTrade path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV TrustTrade return path")
    ...
    reflection_history.to_csv(output / "reflection_history.csv", index=False)
    ...
    action_counts = {
        "buy": int(reflection_history["buy_count"].sum()),
        "hold": int(reflection_history["hold_count"].sum()),
        "sell": int(reflection_history["sell_count"].sum()),
    }
    mean_position = float(reflection_history["mean_absolute_position"].mean())
    mean_risk_cap = float(reflection_history["mean_risk_cap"].mean())
    high_consensus = {
        domain: int(reflection_history[f"high_consensus_count__{domain}"].sum())
        for domain in domain_reports
    }
    verdict = f"""# M049: TrustTrade in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not the native TrustTrade LLM and human-study system.

The reconstruction preserves independent reports in four domains, hybrid semantic/numeric agreement, support/cohesion filtering, deterministic temporal anchoring, sequential trader/risk sizing, carried HOLD decisions, and purged short/long reflection. Twelve JKP report proxies replace unavailable LLM claims. Aggregate actions were {action_counts}; mean absolute position was {mean_position:.3f}, mean risk cap was {mean_risk_cap:.3f}, and high-consensus stock-domain counts were {high_consensus}.
...
This result answers how one transparent TrustTrade-inspired selective-consensus and reflection policy transfers to the common monthly U.S. universe. It does not reproduce the paper's LLM reports, claim embeddings, human decisions, three-stock interface, or 2026 paths. Future-contaminated interface reports and participant outputs were not used.
"""
    (output / "verdict.md").write_text(verdict)
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen TrustTrade recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "report_count": sum(len(reports) for reports in domain_reports.values()),
        "policy_update_months": len(reflection_history),
        "aggregate_actions": action_counts,
        "mean_absolute_position": mean_position,
        "mean_risk_cap": mean_risk_cap,
        "high_consensus_counts": high_consensus,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M049_trusttrade").resolve()
