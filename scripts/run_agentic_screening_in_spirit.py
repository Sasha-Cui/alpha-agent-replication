#!/usr/bin/env python3
"""Run the frozen M050 agentic screening and precision policy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M050 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("agentic screening recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("agentic screening result must remain explicitly in-spirit")
    quant_policy = recipe["consensus_and_quant_policy"]
    if quant_policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("agentic screening recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["linked_news_dataset_audit_path"]: recipe["strict_evidence"]["linked_news_dataset_audit_sha256"],
        recipe["strict_evidence"]["independent_implementation_audit_path"]: recipe["strict_evidence"]["independent_implementation_audit_sha256"],
        recipe["strict_evidence"]["prompt_component_inventory_path"]: recipe["strict_evidence"]["prompt_component_inventory_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned agentic screening evidence changed: {relative}")
    ...
    if audit["attributable_native_implementation_recovered"] is not False:
        raise ValueError("pinned agentic screening release status changed")
    if provenance["arxiv_id"] != "2603.23300" or provenance["current_version"] != "v2":
        raise ValueError("pinned agentic screening paper identity changed")
    ...
    extended_settings["formation_start"] = quant_policy["prehistory_formation_start"]
    features = ["market_equity", "be_me", "ret_12_1", "niq_su", "saleq_su", "ret_1_0", "turnover_126d"]
    print("loading agentic screening prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, screening_history = agentic_screening_precision_scores(
        extended,
        common_start=settings["formation_start"],
        formation_history_months=quant_policy["formation_history_months"],
        minimum_security_history_months=quant_policy["minimum_security_history_months"],
        precision_winsorization=tuple(quant_policy["precision_winsorization"]),
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(screening_history) != len(expected_months):
        raise ValueError("agentic screening reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M050_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete agentic screening path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV agentic screening return path")
    ...
    screening_history.to_csv(output / "screening_history.csv", index=False)
    ...
    action_counts = {
        "buy": int(screening_history["final_buy_count"].sum()),
        "hold": int(screening_history["final_hold_count"].sum()),
        "sell": int(screening_history["final_sell_count"].sum()),
    }
    buy_fallbacks = int(screening_history["buy_union_fallback"].sum())
    sell_fallbacks = int(screening_history["sell_union_fallback"].sum())
    conflicts = int(screening_history["conflict_count"].sum())
    verdict = f"""# M050: Agentic AI Screening in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not the native LLM-S/FinBERT and precision-matrix system.

The reconstruction preserves two specialized screens, size/value/momentum LLM-S logic, monthly news-sentiment decisions, intersection consensus with union fallback, screening before risk weighting, a 180-month precision window, and the maximum-Sharpe objective. A deterministic earnings-news score replaces unreleased FinBERT output and diagonal inverse variance replaces unreleased high-dimensional precision matrices. Aggregate actions were {action_counts}; buy and sell union fallbacks occurred {buy_fallbacks} and {sell_fallbacks} months, with {conflicts} stock-month conflicts assigned HOLD.
...
This result answers how one transparent two-agent screening-before-weighting policy transfers to the common monthly U.S. universe. It does not reproduce annual LLM outputs, FinBERT probabilities, native covariance estimators, optimized weights, or the paper's S&P 500 claims. The linked news text and unaffiliated rewrite were not used.
"""
    (output / "verdict.md").write_text(verdict)
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen agentic screening recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "screening_agent_count": 2,
        "formation_history_months": quant_policy["formation_history_months"],
        "policy_update_months": len(screening_history),
        "aggregate_actions": action_counts,
        "buy_union_fallback_months": buy_fallbacks,
        "sell_union_fallback_months": sell_fallbacks,
        "conflict_stock_months": conflicts,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M050_agentic_screening").resolve()
