#!/usr/bin/env python3
"""Run the frozen M016 MarketSenseAI-inspired monthly signal on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M016 in-spirit run already exists; do not silently overwrite it")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["result_ledger_path"]: recipe["strict_evidence"]["result_ledger_sha256"],
    }
    ...
    scores, policy_history = marketsenseai_signal_scores(
        extended,
        specialists,
        common_start=settings["formation_start"],
        reliability_months=policy["reliability_window_months"],
    ...
    holdings_path = private / "M016_formation_holdings.parquet"
    ...
    verdict = f"""# M016: MarketSenseAI 2.0 in-spirit reconstruction on monthly U.S./JKP data
...
At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks. Diagnostic actions total Buy={action_counts['buy']}, Sell={action_counts['sell']}, Hold={action_counts['hold']}.
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M016_marketsenseai").resolve()
