...
def verify_prompt_replay(run_dir: Path) -> dict:
    ...
    if float(manifest.get("costs", {}).get("primary_cost_bps", -1.0)) != 1.0:
        raise RuntimeError("GuruAgents prompt replay does not use the paper-declared 1-bp cost")
...
def main() -> int:
    ...
    prompt_best = prompt_economics[
        ...
        & prompt_economics["return_basis"].eq("net_costed")
    ...
    prompt_table_lines = [
        ...
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{Prompt-replayed GuruAgents factor attribution. Every row uses the same 12 paths and 33 realized months, net of 1-bp one-way costs.}",
