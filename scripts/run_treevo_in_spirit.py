#!/usr/bin/env python3
"""Run the frozen M028 TreEvo-inspired hierarchical search on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M028 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("TreEvo recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("TreEvo result must remain explicitly in-spirit")
    evolution = recipe["evolution_policy"]
    if evolution["final_common_returns_used_for_search_or_selection"] is not False:
        raise ValueError("TreEvo recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["operator_inventory_path"]: recipe["strict_evidence"]["operator_inventory_sha256"],
        recipe["strict_evidence"]["method_audit_path"]: recipe["strict_evidence"]["method_audit_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned TreEvo evidence changed: {relative}")
    provenance = json.loads((root / recipe["strict_evidence"]["source_provenance_path"]).read_text())
    if provenance["versions"]["v2"]["main_sha256"] != recipe["paper_source"]["source_main_sha256"]:
        raise ValueError("pinned TreEvo paper identity changed")
    ...
    extended_settings["formation_start"] = evolution["prehistory_formation_start"]
    seed_features = [
        (item["column"], int(item["sign"]))
        for item in recipe["seed_features"]
    ]
    features = [feature for feature, _ in seed_features]
    print("loading TreEvo prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, search_history, search_summary = treevo_evolution_scores(
        extended,
        seed_features,
        common_start=settings["formation_start"],
        training_start=evolution["search_training_start"],
        training_end=evolution["search_training_end"],
        validation_start=evolution["search_validation_start"],
        validation_end=evolution["search_validation_end"],
        population_size=evolution["population_size"],
        evaluation_budget=evolution["evaluation_budget"],
        offspring_generations=evolution["offspring_generations"],
        offspring_per_generation=evolution["offspring_per_generation"],
        operator_rotation=tuple(evolution["operator_rotation"]),
        mutation_probabilities=tuple(evolution["mutation_probabilities"].values()),
        random_seed=evolution["random_seed"],
        maximum_initial_depth=recipe["tree_grammar"]["maximum_initial_depth"],
        complexity_penalty_per_extra_node=evolution["complexity_penalty_per_extra_node"],
    )
    ...
    if formed["month"].nunique() != len(expected_months):
        raise ValueError("TreEvo reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M028_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete TreEvo path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV TreEvo return path")
    ...
    invalid_candidates = int((~search_history["valid_candidate"]).sum())
    verdict = f"""# M028: TreEvo in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not a native Qwen/TreEvo replication.

The reconstruction preserves a population of ten hierarchical factor trees, exactly 200 candidate evaluations, rotating crossover/mutation/pruning generations, scoped root/internal/fine mutation, parent-plus-offspring survival, and parsimony. All search fitness uses 96 pre-common months ending June 1997; the final population is selected on 24 subsequent validation months ending June 1999. The chosen tree is `{search_summary['selected_expression']}` with {search_summary['selected_node_count']} nodes, training-oriented RankIC {search_summary['selected_training_mean_rankic']:.4f}, and validation RankIC {search_summary['selected_validation_mean_oriented_rankic']:.4f}. {invalid_candidates} degenerate candidate evaluations were assigned fail-closed fitness and could not win.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen TreEvo recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "population_size": evolution["population_size"],
        "evaluation_budget": len(search_history),
        "invalid_candidate_evaluations": invalid_candidates,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M028_treevo").resolve()
