#!/usr/bin/env python3
"""Run the frozen M029 TradingGroup-inspired reflected agent chain on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M029 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("TradingGroup recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("TradingGroup result must remain explicitly in-spirit")
    reflection = recipe["reflection_and_risk_policy"]
    if reflection["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("TradingGroup recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["formula_inventory_path"]: recipe["strict_evidence"]["formula_inventory_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned TradingGroup evidence changed: {relative}")
    provenance = json.loads((root / recipe["strict_evidence"]["source_provenance_path"]).read_text())
    if provenance["arxiv"]["id"] != recipe["paper_source"]["arxiv_id"]:
        raise ValueError("pinned TradingGroup paper identity changed")
    ...
    extended_settings["formation_start"] = reflection["prehistory_formation_start"]
    information_agents = {
        agent: [(item["column"], int(item["sign"])) for item in specifications]
        for agent, specifications in recipe["information_agents"].items()
    }
    features = list(
        dict.fromkeys(
            [
                feature
                for specifications in information_agents.values()
                for feature, _ in specifications
            ]
            + reflection["risk_features"]
            + reflection["safety_features"]
        )
    )
    print("loading TradingGroup prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, reflection_history = tradinggroup_reflection_scores(
        extended,
        information_agents,
        reflection["styles"],
        risk_features=reflection["risk_features"],
        safety_features=reflection["safety_features"],
        common_start=settings["formation_start"],
        forecast_reflection_months=reflection["forecast_reflection_months"],
        minimum_forecast_rankic_months=reflection["minimum_forecast_rankic_months"],
        forecast_reliability_temperature=reflection["forecast_reliability_temperature"],
        style_reflection_months=reflection["style_reflection_months"],
        minimum_style_rankic_months=reflection["minimum_style_rankic_months"],
    )
    ...
    if (
        ...
        or len(reflection_history) != len(expected_months)
    ):
        raise ValueError("TradingGroup reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M029_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete TradingGroup path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV TradingGroup return path")
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen TradingGroup recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "agent_count": len(recipe["five_agent_roles"]),
        "policy_update_months": len(reflection_history),
        "style_selection_counts": style_counts,
        "risk_intercepted_recommendations": intercepted,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M029_tradinggroup").resolve()
