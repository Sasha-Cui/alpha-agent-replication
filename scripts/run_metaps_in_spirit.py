#!/usr/bin/env python3
"""Run the frozen M060 MetaPS-inspired V3 strategy router on U.S./JKP."""
...
from alpha_evolve.in_spirit import metaps_v3_router_scores
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M060 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("MetaPS recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("MetaPS result must remain explicitly in-spirit")
    supervision_policy = recipe["supervision_policy"]
    if supervision_policy["final_common_or_current_forward_return_used_for_router_selection"] is not False:
        raise ValueError("MetaPS recipe permits final-result selection")
    ...
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned MetaPS evidence changed: {relative}")
    ...
    if audit["attributable_metaps_implementation_found"] is not False:
        raise ValueError("pinned MetaPS implementation status changed")
    if provenance["arxiv_id"] != "2606.22385":
        raise ValueError("pinned MetaPS paper identity changed")
    ...
    extended_settings["formation_start"] = supervision_policy["prehistory_formation_start"]
    strategy_library = recipe["strategy_library"]
    features = list(
        dict.fromkeys(
            [feature for strategy in strategy_library.values() for feature in strategy["inputs"] if feature != "weight"]
            + recipe["router_state_input_columns"]
        )
    )
    print("loading MetaPS prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, router_history, counterfactual_rollouts = metaps_v3_router_scores(
        extended,
        strategy_library,
        common_start=settings["formation_start"],
        router_training_months=supervision_policy["router_training_months"],
        reward_purge_months=supervision_policy["reward_purge_months"],
        v2_horizons_months=supervision_policy["v2_horizons_months"],
        v2_horizon_weights=supervision_policy["v2_horizon_weights"],
        v3_short_horizon_quality_weight=supervision_policy["v3_short_horizon_quality_weight"],
        v3_medium_horizon_quality_weight=supervision_policy["v3_medium_horizon_quality_weight"],
        v3_active_strategy_bonus=supervision_policy["v3_active_strategy_bonus"],
        v3_frequency_balance_penalty=supervision_policy["v3_frequency_balance_penalty"],
        candidate_budget=supervision_policy["candidate_budget"],
        router_ridge_penalty=supervision_policy["router_ridge_penalty"],
        inner_tail_fraction=supervision_policy["inner_tail_fraction"],
        inner_minimum_side=supervision_policy["inner_minimum_side"],
        inner_cost_bps_one_way=supervision_policy["inner_cost_bps_one_way"],
        exposure_buckets=supervision_policy["coarse_exposure_buckets"],
        margin_thresholds=supervision_policy["router_margin_thresholds"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(router_history) != len(expected_months):
        raise ValueError("MetaPS reconstruction does not cover the fixed formation calendar")
    if len(counterfactual_rollouts) != extended["month"].nunique():
        raise ValueError("MetaPS reconstruction has an incomplete rollout trace")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M060_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete MetaPS path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV MetaPS return path")
    ...
    router_history.to_csv(output / "router_history.csv", index=False)
    counterfactual_rollouts.to_csv(output / "counterfactual_rollouts.csv", index=False)
    ...
    strategy_counts = router_history["selected_strategy"].value_counts().to_dict()
    exposure_counts = router_history["exposure_bucket"].value_counts().to_dict()
    relevance_match_rate = float(
        router_history["selected_strategy"].eq(router_history["top_relevance_strategy"]).mean()
    )
    average_router_confidence = float(router_history["router_confidence"].mean())
    average_router_margin = float(router_history["router_margin"].mean())
    verdict = f"""# M060: MetaPS in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not the native MetaPS Qwen router or action path.

The reconstruction preserves ten named strategy programs, ranked candidate context, counterfactual V1/V2 rollouts, V3 return-aware label balancing, strategy-level routing, and coarse exposure buckets. The rolling ridge router selected strategies with counts {strategy_counts}, exposure counts {exposure_counts}, mean confidence {average_router_confidence:.3f}, mean top-two margin {average_router_margin:.3f}, and a {relevance_match_rate:.1%} match to the current lightweight relevance leader. Every decision used 120 fully realized historical labels with a six-month purge. No current forward return or final common result selected the router action.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen MetaPS recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "router_update_months": len(router_history),
        "counterfactual_rollout_months": len(counterfactual_rollouts),
        "selected_strategy_counts": strategy_counts,
        "exposure_bucket_counts": exposure_counts,
        "current_relevance_match_rate": relevance_match_rate,
        "average_router_confidence": average_router_confidence,
        "average_router_margin": average_router_margin,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M060_metaps").resolve()
