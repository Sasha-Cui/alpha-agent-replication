#!/usr/bin/env python3
"""Run the frozen M066 AlphaAgentEvo-inspired factor policy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M066 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("AlphaAgentEvo recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("AlphaAgentEvo result must remain explicitly in-spirit")
    agent_policy = recipe["agentic_rl_policy"]
    portfolio_policy = recipe["portfolio_policy"]
    if portfolio_policy["common_or_post_training_returns_used_for_policy_or_alpha_selection"] is not False:
        raise ValueError("AlphaAgentEvo recipe permits common-period feedback")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["candidate_release_path"]: recipe["strict_evidence"]["candidate_release_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned AlphaAgentEvo evidence changed: {relative}")
    ...
    if audit["attributable_alphaagentevo_code_recovered"] is not False:
        raise ValueError("pinned AlphaAgentEvo implementation status changed")
    if provenance["openreview"]["forum_id"] != "lNmZrawUMu":
        raise ValueError("pinned AlphaAgentEvo paper identity changed")
    ...
    extended_settings["formation_start"] = agent_policy["prehistory_formation_start"]
    grammar = recipe["factor_grammar"]
    features = grammar["seed_features"]
    reward_spec = recipe["hierarchical_reward"]
    reward_weights = {
        name: reward_spec[f"{name}_weight"]
        for name in ("validity", "quality", "diversity", "novelty", "tool_efficiency")
    }
    print("loading AlphaAgentEvo training and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, elite_pool, tool_history, policy_history = alphaagentevo_policy_scores(
        extended,
        features,
        reward_weights,
        common_start=settings["formation_start"],
        training_months=agent_policy["training_months"],
        training_steps=agent_policy["training_steps"],
        offspring_tool_calls_per_step=agent_policy["offspring_tool_calls_per_step"],
        maximum_tool_calls=agent_policy["maximum_tool_calls"],
        elite_pool_capacity=agent_policy["elite_pool_capacity"],
        sampling_temperature=agent_policy["sampling_temperature"],
        group_relative_learning_rate=agent_policy["group_relative_learning_rate"],
        policy_logit_clip=agent_policy["policy_logit_clip"],
        random_seed=agent_policy["random_seed"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or scores.notna().sum() != len(formed):
        raise ValueError("AlphaAgentEvo reconstruction does not cover the fixed formation calendar")
    if len(elite_pool) != agent_policy["elite_pool_capacity"]:
        raise ValueError("AlphaAgentEvo reconstruction has an incomplete elite pool")
    if len(tool_history) != agent_policy["training_steps"] * agent_policy["maximum_tool_calls"]:
        raise ValueError("AlphaAgentEvo reconstruction has an incomplete tool trajectory")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M066_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete AlphaAgentEvo path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV AlphaAgentEvo return path")
    ...
    elite_pool.to_csv(output / "elite_pool.csv", index=False)
    tool_history.to_csv(output / "tool_history.csv", index=False)
    ...
    selected_alpha = str(elite_pool.loc[elite_pool["selected_final"], "alpha"].iloc[0])
    selected_reward = float(elite_pool.loc[elite_pool["selected_final"], "reward"].iloc[0])
    operator_call_counts = tool_history["operator"].value_counts().to_dict()
    reward_component_means = {
        name.removeprefix("reward__"): float(tool_history[name].mean()) for name in tool_history.filter(like="reward__")
    }
    initial_best_reward = float(policy_history.iloc[0]["best_reward"])
    final_best_reward = float(policy_history.iloc[-1]["best_reward"])
    verdict = f"""# M066: AlphaAgentEvo in-spirit reconstruction on monthly U.S./JKP data
...
The reconstruction preserves 150 persistent policy updates, four factor-evaluation tool calls per step, group-relative advantages, validity/quality/diversity/novelty/tool-efficiency rewards, clipped operator and feature preferences, and a diverse 20-alpha elite pool. The selected alpha was `{selected_alpha}` with hierarchical reward {selected_reward:.3f}; operator calls were {operator_call_counts}, and mean reward components were {reward_component_means}. All policy learning occurred on the 120 pre-common training months.
...
"""
    ...
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen AlphaAgentEvo recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "training_steps": len(policy_history),
        "tool_call_count": len(tool_history),
        "elite_pool_size": len(elite_pool),
        "selected_alpha": selected_alpha,
        "selected_alpha_reward": selected_reward,
        "operator_call_counts": operator_call_counts,
        "reward_component_means": reward_component_means,
        "initial_step_best_reward": initial_best_reward,
        "final_step_best_reward": final_best_reward,
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M066_alphaagentevo").resolve()
