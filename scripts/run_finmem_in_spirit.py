#!/usr/bin/env python3
"""Run the frozen M006 FinMem-inspired layered-memory policy on U.S./JKP."""
...
def evaluate(root: Path, output: Path) -> None:
    if (output / "run_manifest.json").exists():
        raise ValueError("M006 in-spirit run already exists; do not silently overwrite it")
    ...
    if recipe["status"] != "frozen_before_jkp_result":
        raise ValueError("FinMem recipe is not frozen")
    if recipe["fidelity_label"] != "in_spirit_reconstruction":
        raise ValueError("FinMem result must remain explicitly in-spirit")
    policy = recipe["chronological_policy"]
    if policy["final_common_returns_used_for_policy_choice"] is not False:
        raise ValueError("FinMem recipe permits final-result selection")
    ...
    pinned = {
        ...
        recipe["strict_evidence"]["action_metric_path"]: recipe["strict_evidence"]["action_metric_sha256"],
        recipe["strict_evidence"]["configuration_path"]: recipe["strict_evidence"]["configuration_sha256"],
    }
    for relative, expected in pinned.items():
        if digest(root / relative) != expected:
            raise ValueError(f"pinned FinMem evidence changed: {relative}")
    ...
    features = sorted({name for layer in recipe["memory_layers"].values() for name in layer["features"]})
    print("loading FinMem prehistory and common formations", flush=True)
    extended = load_formations(source_path, features, extended_settings)
    scores, policy_history = finmem_layered_scores(
        extended,
        recipe["memory_layers"],
        common_start=settings["formation_start"],
        memory_horizon_months=policy["memory_horizon_months"],
        top_k=policy["top_k_per_layer"],
        trading_days_per_month=policy["trading_days_per_month"],
        risk_adjustment_magnitude=policy["risk_adjustment_magnitude"],
    )
    ...
    if formed["month"].nunique() != len(expected_months) or len(policy_history) != len(expected_months):
        raise ValueError("FinMem reconstruction does not cover the fixed formation calendar")

    private = root / "artifacts/us_jkp_in_spirit/v1"
    private.mkdir(parents=True, exist_ok=True)
    os.chmod(private, 0o700)
    holdings_path = private / "M006_formation_holdings.parquet"
    ...
    if not base.path_status.eq("ok").all():
        ...
        raise ValueError(f"incomplete FinMem path: {bad.to_dict(orient='records')[:5]}")
    ...
    if not np.isfinite(returns).all() or (returns <= -1).any():
        raise ValueError("nonfinite or nonpositive-NAV FinMem return path")
    ...
    policy_history.to_csv(output / "memory_history.csv", index=False)
    ...
    character_counts = policy_history["risk_character"].value_counts()
    character_text = ", ".join(f"{name}={int(count)}" for name, count in character_counts.items())
    verdict = f"""# M006: FinMem in-spirit reconstruction on monthly U.S./JKP data

Status: **completed researcher-authored in-spirit reconstruction**, not a native FinMem replication.

The reconstruction preserves FinMem's shallow, intermediate, and deep memory layers; top-five retrieval; layer-specific recency, relevance, and importance scoring; reflection over current facts and memory votes; and an adaptive risk character. Numeric JKP states and cosine similarity replace unavailable news/filing text and embeddings. Only earlier security outcomes enter memories, and the trailing three-month formation-time market return switches between risk-seeking and risk-averse modes. The unavailable GPT-4-Turbo reflections, trained memory stream, and native action histories are not reproduced.

At 10 bp one-way costs, the 305-month path has CAGR {primary['full_cagr']:.2%}, annualized Sharpe {primary['full_annualized_sharpe']:.3f}, and maximum drawdown {primary['full_maximum_drawdown']:.2%}. Mean monthly traded notional is {primary['average_traded_notional']:.3f}, and minimum signal coverage is {primary['minimum_finite_signal_count']} stocks. Character months are {character_text}.
...
This result answers how one transparent FinMem-inspired layered-memory policy transfers to the common task. It does not reproduce or validate the paper's private memories, LLM decisions, five-trial paths, reported returns, or model-comparison claims.
"""
    (output / "verdict.md").write_text(verdict)
    if frozen_hashes != {"contract": digest(contract_path), "recipe": digest(recipe_path)}:
        raise RuntimeError("frozen FinMem recipe or common contract changed during evaluation")
    outputs = [
        "monthly_returns.csv",
    ...
    manifest = {
        ...
        "memory_layer_count": len(recipe["memory_layers"]),
        "policy_update_months": len(policy_history),
        "risk_character_counts": {str(name): int(count) for name, count in character_counts.items()},
...
def main() -> None:
    ...
    output = (args.output or root / "paper_runs/us_jkp_in_spirit/M006_finmem").resolve()
