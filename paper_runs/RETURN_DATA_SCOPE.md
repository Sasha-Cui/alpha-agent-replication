...
Runtime guardrail: legacy scripts that read paper-shipped returns, official Kenneth French factors, or the older external benchmark factor panel now refuse to run unless `ALLOW_LEGACY_NON_JKP_RETURNS=1` is set. That opt-in is only for explicit non-counting audit reproduction; valid experiments should use `scripts/build_jkp_long_short_returns.py`, `scripts/run_quantevolver_jkp_seed_proxies.py`, and `scripts/evaluate_candidate_returns_jkp.py`.
