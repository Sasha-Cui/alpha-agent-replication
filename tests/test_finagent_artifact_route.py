...
def test_committed_artifact_audit_and_summary_include_correction() -> None:
    ...
    assert ft["public_artifact_listed"]["successes"] == "21"
    assert ft["artifact_reachable_among_all"]["successes"] == "19"
    assert ft["github_head_resolved_among_all"]["successes"] == "19"
    assert ft["static_R3_among_all"]["successes"] == "8"
    ...
    assert correction[0]["system_id"] == "SYS-FIN-AGENT"
...
def test_static_paper_assets_and_claim_hashes_reflect_the_correction() -> None:
    ...
    assert r"\newcommand{\ArtifactCountFT}{21}" in generated
    assert r"\newcommand{\ArtifactRateFT}{31.3\%}" in generated
    assert r"\newcommand{\ReachableArtifactCountFT}{19}" in generated
    assert r"\newcommand{\LicensedArtifactCountFT}{10}" in generated
    assert r"\newcommand{\PinnedRepoCountFT}{19}" in generated
    assert r"\newcommand{\TargetedAuditCount}{27}" in generated
    ...
    assert claims["ArtifactCountFT"]["rendered_value"] == "21"
    ...
    assert claims["TargetedAuditCount"]["rendered_value"] == "27"
