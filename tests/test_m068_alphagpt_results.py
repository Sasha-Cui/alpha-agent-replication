...
OUTPUT = ROOT / "paper_runs/us_jkp_headline/M068_alphagpt"
...
def test_m068_outputs_pin_the_frozen_partial_and_hashes():
    ...
    assert manifest["milestone_id"] == recipe["milestone_id"] == "M068"
    ...
    assert manifest["code_commit"] == "53dd0de7a914d14a359b26ee25a7ea0f089345a2"
    assert manifest["source_formula_id"] == "AGPT-FORM-02"
    assert manifest["source_direction"] == "literal_positive_orientation"
    assert manifest["confirmatory_claim"] is False
    for name, expected in manifest["output_sha256"].items():
        assert sha256(OUTPUT / name) == expected


def test_m068_primary_path_uses_common_calendar_weights_and_accounting():
    ...
    assert path.finite_signal_count.min() == 821
...
def test_m068_volume_price_formula_is_positive_but_not_distinct_from_jkp():
    ...
    assert primary.full_cagr == pytest.approx(0.03733756911546049)
    assert primary.full_annualized_sharpe == pytest.approx(0.28107883564689756)
    assert primary.full_maximum_drawdown == pytest.approx(-0.5027234935060045)
    assert primary.jkp_residual_mean_annualized == pytest.approx(0.03818512629151595)
    assert primary.jkp_residual_t_hac == pytest.approx(1.3533378446895434)
    assert primary.jkp_residual_p_two_sided == pytest.approx(0.17594772422828153)
    assert primary.exploratory_bonferroni69_p == 1.0
    assert primary.average_traded_notional == pytest.approx(0.897792783659274)
...
def test_m068_ledger_records_the_evaluated_formula_partial():
    ledger = json.loads((ROOT / "paper_runs/us_jkp_headline/milestones.json").read_text())
    rows = {row["milestone_id"]: row for row in ledger["milestones"]}
    milestone = rows["M068"]
    assert milestone["status"] == "completed_partial"
    assert milestone["implementation_path"] == "scripts/run_alphagpt_headline.py"
    ...
    assert ledger["progress_summary"]["closed"] >= 68
    assert ledger["progress_summary"]["completed_partial"] >= 15
