...
def test_factfin_is_a_targeted_paper_only_audit_with_zero_native_credit() -> None:
    ledger = rows(ROOT / "paper_runs/submission_evidence/native_fidelity_ledger.csv")
    row = next(item for item in ledger if item["system_id"] == "SYS-FACTFIN")
    ...
    assert row["targeted_execution_audit_status"] == (
        "paper_audit:completed_zero_of_525_numeric_cells_no_public_system_or_benchmark_release"
    )
    assert row["fidelity_class"] == "F0_no_public_artifact"
    note = row["concise_evidence_note"]
    assert "0/525 cells reproduce" in note
    assert "120 direct FactFin cells" in note
    assert "108 unique FactFin measurements" in note
    assert "0/6 Buy-and-Hold cells" in note
    assert "not proof" in note


def test_factfin_paper_route_exposes_the_precise_blocker() -> None:
    routes = rows(
        ROOT / "paper_runs/submission_evidence/replication_scope/paper_evidence_route_ledger.csv"
    )
    row = next(item for item in routes if item["canonical_work_id"] == "CensusArxiv251007920")
    assert row["paper_evidence_route"] == "paper_only_underspecified"
    assert row["system_ids"] == "SYS-FACTFIN; SYS-FINLAKE-BENCH"
    ...
    assert "SYS-FACTFIN:A0_no_public_artifact" in row["precise_native_or_access_blocker"]
    assert "0/525 cells reproduce" in row["precise_native_or_access_blocker"]


def test_static_report_and_claim_ledger_count_factfin_once() -> None:
    generated = (ROOT / "docs/paper/generated_results.tex").read_text(encoding="utf-8")
    assert r"\newcommand{\TargetedAuditCount}{39}" in generated
    claims = {row["macro"]: row for row in rows(ROOT / "paper_runs/submission_evidence/claims.csv")}
    assert claims["TargetedAuditCount"]["rendered_value"] == "39"
    ...
    assert "FactFin" in failures
    assert "0/525 cells reproduce" in failures
    assert "0/6 Buy-and-Hold cells" in failures


def test_paper_manifest_and_routed_summary_share_zero_credit_boundary() -> None:
    data = json.loads(
        (ROOT / "paper_runs/paper_replication_audits/factfin/manifest.json").read_text(
            encoding="utf-8"
        )
    )
