...
def test_timi_is_a_targeted_paper_only_audit_with_zero_native_credit() -> None:
    ledger = rows(ROOT / "paper_runs/submission_evidence/native_fidelity_ledger.csv")
    row = next(item for item in ledger if item["system_id"] == "SYS-TRADE-IN-MINUTES")
    ...
    assert row["targeted_execution_audit_status"] == (
        "paper_audit:completed_zero_of_349_active_empirical_units_zero_of_8_"
        "empirical_panels_listed_broken_supplement_no_attributable_pipeline"
    )
    assert row["fidelity_class"] == "F0_no_public_artifact"
    note = row["concise_evidence_note"]
    assert "0/349 active table units" in note
    assert "0/8 empirical panels" in note
    assert "logical endpoint currently returns HTTP 403" in note
    assert "immutable path returns HTTP 404" in note
    assert "listed but not inspected or presumed absent" in note
    assert "unaffiliated and receive no native credit" in note
    assert "favorable narrative motif" in note


def test_timi_paper_route_exposes_the_precise_blocker() -> None:
    routes = rows(
        ROOT / "paper_runs/submission_evidence/replication_scope/paper_evidence_route_ledger.csv"
    )
    row = next(item for item in routes if item["canonical_work_id"] == "CensusArxiv251004787")
    ...
    assert "SYS-TRADE-IN-MINUTES:A0_no_public_artifact" in blocker
    assert "0/349 active table units" in blocker
...
def test_static_report_and_claim_ledger_count_timi_once() -> None:
    ...
    assert r"\newcommand{\TargetedAuditCount}{58}" in generated
    claims = {row["macro"]: row for row in rows(ROOT / "paper_runs/submission_evidence/claims.csv")}
    assert claims["TargetedAuditCount"]["rendered_value"] == "58"
    ...
    assert "Trade in Minutes" in failures
    assert "0/349 active table units" in failures


def test_timi_manifest_preserves_the_listed_but_unrecovered_boundary() -> None:
    manifest = json.loads(
        (ROOT / "paper_runs/paper_replication_audits/timi/manifest.json").read_text(
            encoding="utf-8"
        )
    )
    assert manifest["official_pdf_and_source_recovered"] is True
    assert manifest["official_pages_visually_checked"] == 33
    assert manifest["rebuilt_pages_visually_checked"] == 33
    assert manifest["official_supplement_listed"] is True
    assert manifest["official_supplement_recovered"] is False
    assert manifest["attributable_timi_code_recovered"] is False
