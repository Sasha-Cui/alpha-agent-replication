...
def test_native_ledger_credits_outputs_but_not_end_to_end_reproduction() -> None:
    ...
    assert "29/42 displayed units verify" in note
    assert "36/42 are checked" in note
    assert "8/42 paper-internal" in note
    assert "five rolling claims conflict" in note
    assert "All eight standard 20-day" in note
    assert "Across 1,160 longer-window" in note
...
def test_static_paper_assets_reflect_raptor_correction() -> None:
    ...
    assert "29/42 displayed units verify" in failure_table
    assert "36/42 are checked" in failure_table
