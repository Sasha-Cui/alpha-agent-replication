...
def test_committed_artifact_audit_and_summary_include_raptor() -> None:
    ...
    assert ft["artifact_reachable_among_all"]["successes"] == "20"
    assert ft["github_head_resolved_among_all"]["successes"] == "20"
    assert ft["static_R3_among_all"]["successes"] == "9"
...
def test_static_paper_assets_reflect_raptor_correction() -> None:
    ...
    assert r"\newcommand{\ReachableArtifactCountFT}{20}" in generated
    assert r"\newcommand{\LicensedArtifactCountFT}{11}" in generated
    assert r"\newcommand{\PinnedRepoCountFT}{20}" in generated
    assert r"\newcommand{\ArtifactTierSummaryFT}{\artifacttier{R0}: 47, \artifacttier{R1}: 6, \artifacttier{R2}: 5, \artifacttier{R3}: 9}" in generated
    assert r"\newcommand{\NativeDatedOutputCount}{4}" in generated
    assert r"\newcommand{\TargetedAuditCount}{28}" in generated
