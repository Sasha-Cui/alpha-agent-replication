...
def test_published_and_extended_result_censuses_are_complete_and_uncredited() -> None:
    ...
    assert len(v1) == 956
    assert len(v3) == 1312
    assert {row["paper_result_credit"] for row in published + v1 + v3} == {"False"}
    assert {row["native_alpha_jungle_result_credit"] for row in published + v1 + v3} == {"False"}
...
def test_method_and_claim_audits_retain_the_decisive_blockers() -> None:
    ...
    metric_lineage = next(row for row in methods if row["dimension"] == "v1 AR to v3/final AER lineage")
...
def test_dynamic_pinned_sources_and_parsers_when_available() -> None:
    ...
    assert (len(published), len(v1), len(v3), len(lineage), len(costs), len(formulas)) == (
        64,
        956,
        1312,
        932,
