...
OUTPUT = ROOT / "paper_runs/us_jkp_headline/M021_alpha_jungle"
...
def test_m021_outputs_reconstruct_fixed_component_and_hashes():
    ...
    assert manifest["milestone_id"] == recipe["milestone_id"] == "M021"
    assert manifest["candidate_id"] == recipe["candidate_id"] == "alpha_jungle_volume_ma_diff"
...
def test_m021_diagnostics_and_claim_boundary_are_explicit():
    ...
    assert "LLM-guided Monte Carlo tree search" in report
    assert "MCTS search; LLM prompts/responses" in report
    assert "QuantAgent" not in report
    assert "two-bar-lag" not in report


def test_m021_is_closed_as_an_evaluated_partial():
    ledger = json.loads((ROOT / "paper_runs/us_jkp_headline/milestones.json").read_text())
    rows = {row["milestone_id"]: row for row in ledger["milestones"]}
    assert rows["M021"]["status"] == "completed_partial"
    assert ledger["progress_summary"]["closed"] >= 21
    assert ledger["progress_summary"]["completed_partial"] >= 3
