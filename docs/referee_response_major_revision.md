...
We added `mapping_audit.csv`, a 62-row ledger containing every source, formula, portfolio rule, source-support assessment for ingredients/signs/weighting, fidelity tier, omitted native components, freeze timestamp/hash, outcome-blindness field, second-coder field, and existing alternative-mapping status. Forty-nine mappings are narrative translations, 12 have partial support, and one is a released seed expression. The mappings were frozen after U.S. outcomes had been inspected and had no independent second coder. We now state that inference is conditional on the chosen formulas. A limited 144-combination sensitivity varies the five sources with existing alternatives; it is labeled a lower bound, not a substitute for independent coding.
