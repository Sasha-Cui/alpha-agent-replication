...
The revision now cites every one of the 98 screened works in the main paper, including all 29 exclusions. A generated 98-entry BibTeX database and a human-readable full-corpus bibliography account for all retained and excluded works. A machine-readable 98-row evidence waterfall records each work's canonical identifier, preferred citation, screen decision, code-audit role, reconstruction status, mapping count, fidelity class, and negative-inference boundary. The manuscript no longer relies on an unavailable supplement for the census bibliography. Its final reference list contains 108 entries: all 98 corpus works plus ten methodological and benchmark references.

The paper now reconciles every empirical denominator. The 98 works split into 69 retained methods and 29 screened-out benchmarks, comparators, audits, or adjacent works. The 69 retained works split into 40 works receiving 50 good-faith common-task mappings and 29 availability-only works receiving no performance inference. The 40 reconstructed works split into five works supporting 13 source-grounded component tests and 35 works supporting 37 narrative-only favorable stress tests. Twelve additional mappings from 11 screened-out diagnostic sources yield the prespecified 62-portfolio family. The code audit overlaps this waterfall: eight retained works plus six diagnostic works produce 14 attempts, zero native-agent replications, and one released-seed-expression adaptation.

## Minor comments

- The 67 entries are now explicitly described as system lineages in an availability census, not 67 replications or necessarily 67 one-to-one papers. They resolve to 69 retained works, while the 103 pre-trim lineages resolve to 98 works after duplicate publication manifestations are collapsed. All 98 are cited in the paper.
- The 98-work corpus, 69 retained works, 67 retained lineages, 40 reconstructed works, 50 retained-work mappings, 12 diagnostic mappings, 51 mapped source indices, and 62-portfolio family are reconciled in the first methods subsection and waterfall figure.
