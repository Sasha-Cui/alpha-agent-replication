...
\midrule
AlphaAgent \cite{TangEtAl2025AlphaAgent} & China A-share factor-mining code & No shipped dated strategy return series. \\
...
ContestTrade \cite{ZhaoEtAl2025ContestTrade} & Sequential multi-agent trading code & No replay adapter or shipped common-task returns. \\
...
FinCon \cite{YuEtAl2024FinCon} & Sequential multi-agent trading system & Available implementation was unofficial/ambiguous and task-incompatible. \\
GuruAgents \cite{KimEtAl2025GuruAgents} & Nasdaq-100 guru-persona portfolios & Shipped non-JKP backtest is inspectable but incompatible with the common task. \\
...
QuantAgent \cite{XiongEtAl2025QuantAgent} & Hourly/4-hour HFT across stocks, futures, and crypto & Intraday native task cannot be represented by monthly JKP data. \\
QuantEvolver \cite{ZhangEtAl2026QuantEvolver} & Factor-discovery framework plus seed expressions & One released seed expression is testable; $\alpha=0.33\%$, $t=0.65$. Not a trained-agent run. \\
R\&D-Agent-Quant \cite{YangEtAl2025RDAgent} & General R\&D-Agent framework and quant workflow & No shipped U.S. candidate returns or completed JKP adapter. \\
...
Trading-R1 \cite{XiaoEtAl2025TradingR1} & RL-trained trading-reasoning system & Public repository snapshot is a placeholder without executable system code. \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
EFS \cite{LuoEtAl2025EFS} (3) & LLM evolutionary factor search for sparse top-$m$ portfolios on Fama--French and US50/HSI45/CSI300 datasets. & Momentum/low-volatility/breakout score; reversal/low-noise score; and a sparse equal-weight top-five variant. \\
QuantEvolver \cite{ZhangEtAl2026QuantEvolver} (1) & Reinforcement fine-tuning generates executable factor-DSL expressions across market benchmarks. & The released risk-adjusted-momentum seed $\mathrm{ret}_{12,1}/|\mathrm{rvol}_{252}|$; portfolio formation remains ours. \\
Alpha-Jungle \cite{ShiEtAl2025AlphaJungle} (2) & LLM-guided MCTS searches symbolic stock factors using price, volume, and volatility operators. & A price/volume/momentum score and a trend-after-volatility-compression score based on documented examples. \\
FAMA \cite{LiEtAl2024FAMA} (1) & Neural-symbolic factor mining evaluated by RankIC/RankICIR and an S\&P 500 investment simulation. & A simple interpretable value--momentum--profitability score motivated by the paper's factor examples. \\
GuruAgents \cite{KimEtAl2025GuruAgents} (6) & Prompt-guided agents emulate five named investor philosophies in a Nasdaq-100 backtest. & Graham, Buffett, Greenblatt, Piotroski, and Altman characteristic screens plus their equal-weight ensemble. \\
\bottomrule
\end{tabular}
\end{table*}
...
The submission package is designed to make the audit inspectable without redistributing restricted security-level data. It contains the complete 67-lineage bibliography and primary-record URLs; the exact 14-code-attempt inventory; the five-paper, 13-component source-grounded inventory; the good-faith reconstruction protocol; the 55-source ledger; all 62 formulas; source-supported content, researcher choices, favorable implementation rules, anti-strawman role, and negative-evidence boundary; aggregate monthly U.S. candidate and factor returns; broad-factor residuals; international event forensics; scripts; tests; and a SHA-256 inventory. The row-level ledger states explicitly that mappings were not outcome-blind and had no second coder. Restricted raw market data are referenced by source path and input hash rather than redistributed.
