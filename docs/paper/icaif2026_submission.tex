...
The submission package is designed to make the audit inspectable without redistributing restricted security-level data. It contains the good-faith reconstruction protocol; the 67-system census; the 55-source ledger; all 62 formulas; source-supported content, researcher choices, favorable implementation rules, task-card status, anti-strawman role, and negative-evidence boundary; the freeze timestamp and hash; aggregate monthly U.S. candidate and factor returns; broad-factor residuals; multiplicity results; international event forensics; scripts; tests; and a SHA-256 inventory. The row-level ledger states explicitly that mappings were not outcome-blind and had no second coder. Restricted raw market data are referenced by source path and input hash rather than redistributed.
