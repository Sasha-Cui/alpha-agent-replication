...
| Published-result conformance audits | Current: fail-closed audits under `paper_runs/paper_replication_audits/`, including FLAG-Trader's 360 result cells, 48-dimension method specification, ACL/arXiv source comparison, and author-linked InvestorBench baseline check | `scripts/audit_flag_trader_paper.py` and the other `scripts/audit_*_paper.py` runners | `paper_runs/prompt_replay/guruagents/paper_table_conformance/`, `paper_runs/paper_replication_audits/` | Pinned official papers and public source workbooks/code/data | Missing native outputs remain unverifiable; static, document, baseline, and component evidence is never promoted to a full-paper replication |
...
python scripts/audit_alpha_r1_paper.py
python scripts/audit_flag_trader_paper.py
```
